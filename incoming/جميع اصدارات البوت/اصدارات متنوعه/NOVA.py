...
بوت سكالبينج Spot تلقائي على Binance Spot Testnet، مع اختيار ديناميكي لأفضل الأزواج.

الاستراتيجية المطبقة:
1) Macro Trend: تعتمد أهلية العملة على زخمها وبنيتها هي، ويجب أن يكون
   إغلاقها فوق EMA200 على 5m فقط؛ لا يوجد شرط اتجاه خارجي على BTC.
2) Demand Setup: شمعتان خضراوان متتاليتان على الأقل مع BOS أو Order Block/FVG صاعدة.
...
4) الحماية: OCO فوري بعد الامتلاء، وقف ثابت أسفل منطقة الطلب وهدف عند Swing High.
   عند وصول السعر إلى Entry + 1.5×ATR(1m) يلغي البوت الحماية الثابتة ويضع OCO جديداً بوقف متحرك.
...
لا يوجد ضمان للربح؛ ابدأ بـ Spot Testnet واختبر السلوك قبل أي أموال حقيقية.
"""
...
LEADERS = ["BTCUSDT"]  # القائد الوحيد
TOP_N_SYMBOLS = env_int("TOP_N_SYMBOLS", 50)
UNIVERSE_REFRESH_EVERY = env_int("UNIVERSE_REFRESH_EVERY", 900)
FIXED_SYMBOLS = [x.strip().upper() for x in os.getenv("SCALP_SYMBOLS", "").split(",") if x.strip()]
SYMBOLS = []
for _symbol in LEADERS + FIXED_SYMBOLS:
    if _symbol not in SYMBOLS:
        SYMBOLS.append(_symbol)

INTERVAL_SECONDS = {"1m": 60, "5m": 300, "15m": 900, "30m": 1800}
KLINE_LIMIT = min(1000, max(600, env_int("KLINE_LIMIT", 1000)))
MIN_CLOSED_CANDLES = max(400, env_int("MIN_CLOSED_CANDLES", 400))
...
MACD_SIGNAL = 4
ATR_PERIOD = 14
MIN_GREEN_CANDLES = env_int("MIN_GREEN_CANDLES", 2)
MAX_GREEN_CANDLES = env_int("MAX_GREEN_CANDLES", 8)
BOS_MIN_PCT = env_float("BOS_MIN_PCT", 0.00)
MIN_FVG_PCT = env_float("MIN_FVG_PCT", 0.00)
ZONE_MAX_AGE_CANDLES = env_int("ZONE_MAX_AGE_CANDLES", 250)
TOUCH_LOOKBACK_BARS = env_int("TOUCH_LOOKBACK_BARS", 60)
FIB_REQUIRED = env_bool("FIB_REQUIRED", False)  # Fibonacci تأكيد إضافي لا حاجز وحيد
MIN_VOLUME_RATIO = env_float("MIN_VOLUME_RATIO", 0.70)
FLASH_MOVE_PCT = env_float("FLASH_MOVE_PCT", 2.8)
FLASH_RANGE_ATR = env_float("FLASH_RANGE_ATR", 5.5)
MIN_24H_QUOTE_VOLUME = env_float("MIN_24H_QUOTE_VOLUME", 500_000)
MAX_SPREAD_PCT = env_float("MAX_SPREAD_PCT", 0.20)
MIN_BOOK_IMBALANCE = env_float("MIN_BOOK_IMBALANCE", 0.35)
MIN_SCORE = env_float("MIN_SCORE", 20.0)
...
TAKE_RR_MIN = env_float("TAKE_RR_MIN", 0.20)
...
# التشغيل والملفات
SCAN_EVERY = env_int("SCAN_EVERY", 7)
...
KLINE_TTL = {"1m": 10, "5m": 30, "15m": 90, "30m": 180}
LAST_UNIVERSE_REFRESH = 0.0
ENTRY_LOCK = threading.Lock()
# حجز قيمة الدخول قبل إطلاق عدة خيوط، حتى لا ترى كل الخيوط الرصيد نفسه
# وتتعهد بأموال أكثر من الرصيد المتاح.
ENTRY_RESERVATIONS = {}
RESERVED_QUOTE = Decimal("0")
MAX_ENTRY_WORKERS = max(1, env_int("MAX_ENTRY_WORKERS", 6))
SCAN_WORKERS = max(4, min(16, env_int("SCAN_WORKERS", 12)))
ENTRY_SEMAPHORE = threading.BoundedSemaphore(MAX_ENTRY_WORKERS)
HTTP_CONCURRENCY = max(2, env_int("HTTP_CONCURRENCY", 20))
HTTP_LIMITER = threading.BoundedSemaphore(HTTP_CONCURRENCY)
MAX_KLINE_CACHE_ENTRIES = max(64, TOP_N_SYMBOLS * 4)
...
class BinanceClient:
    ...
    def new_oco(self, params):
        # واجهة OCO الحديثة: تستخدم above*/below*.
        return self.signed("POST", "/api/v3/orderList/oco", params)

    def new_oco_legacy(self, params):
        # الواجهة القديمة فقط: تستخدم price/stopPrice/stopLimitPrice.
...
def load_symbol_rules(payload):
    ...
    log(f"قواعد Binance USDT: {len(SYMBOL_RULES)} زوجاً متاحاً للاختيار")


def refresh_universe(force=False):
    """يختار أفضل الأزواج المتاحة ديناميكياً حسب حجم التداول 24 ساعة."""
    global SYMBOLS, LAST_UNIVERSE_REFRESH
    now = time.time()
    if not force and now - LAST_UNIVERSE_REFRESH < UNIVERSE_REFRESH_EVERY and len(SYMBOLS) >= TOP_N_SYMBOLS:
        return SYMBOLS
    try:
        ...
        chosen = []
        if LEADERS[0] in SYMBOL_RULES and LEADERS[0] in tickers and SYMBOL_RULES[LEADERS[0]].get("status") == "TRADING":
            chosen.append(LEADERS[0])
        for _, symbol in candidates:
            if symbol not in chosen:
                chosen.append(symbol)
            if len(chosen) >= max(1, TOP_N_SYMBOLS):
                break
        if chosen:
            SYMBOLS = chosen
            LAST_UNIVERSE_REFRESH = now
            log("أفضل الأزواج حسب حجم التداول 24س: " + ", ".join(SYMBOLS))
        return SYMBOLS
    except BinanceError as exc:
        log(f"تعذر تحديث قائمة أفضل الأزواج: {exc}")
        return SYMBOLS
...
def fetch_frame_cached(symbol, interval, force=False):
    ...
    with KLINE_CACHE_LOCK:
        ...
        if cached and not force and now - cached["time"] < KLINE_TTL.get(interval, 30):
            return cached["frame"]
...
def fetch_frames(symbol, force=False):
    frames = {}
    for interval in ("1m", "5m", "15m"):
        frames[interval] = fetch_frame_cached(symbol, interval, force=force)
...
def book_stats(symbol):
    ...
    # دفتر أوامر Testnet اصطناعي؛ لا نجعله حاجزاً ولا نطلب depth إضافياً.
...
def macro_trend_ok(asset_frames, leader_frames=None):
    """فلتر ذاتي للعملة على 5m فقط؛ BTC ليس شرطاً لدخول العملات البديلة."""
    frame = asset_frames.get("5m") if asset_frames else None
    if not frame or not frame["quality"]["ok"]:
        return False, "جودة 5m للعملة غير صالحة"
    if frame["ema200"] is None or frame["close"] <= frame["ema200"]:
        return False, "العملة تحت EMA200 على 5m"
    return True, "5m فوق EMA200"
...
def detect_demand_zone(frame):
    """يبحث عن أحدث اندفاع أخضر متصل، كسر هيكل، وFVG أو Order Block صاعد."""
    if not frame:
        return None
    ...
    for end in range(first_end, last_start, -1):
        ...
        for i in range(start, end - 1):
            # SMC FVG: low of candle 3 remains above high of candle 1.
            if l[i + 2] > h[i] * (1.0 + MIN_FVG_PCT / 100.0):
                fvg = {"low": h[i], "high": l[i + 2], "index": i}
        # إذا لم توجد FVG، نقبل آخر Order Block هابط قبل الاندفاع
        # بشرط أن يتبعه اندفاع صاعد من شمعتين أو أكثر.
        if not fvg:
            for ob_index in range(start - 1, max(0, start - 6), -1):
                if c[ob_index] < o[ob_index]:
                    fvg = {"low": l[ob_index], "high": h[ob_index], "index": ob_index}
        ...
        return {
            ...
            "smc_type": smc_type, "target_high": impulse_high, "impulse_low": impulse_low,
...
def zone_touched(zone, signal_frame):
    """يتحقق من ملامسة المنطقة مع Buffer أمامي 0.25% لتقليل تفويت الدخول."""
    ...
    for i in range(start, len(lows)):
        zone_hit = lows[i] <= zone["zone_high"] * proximity and highs[i] >= zone["zone_low"] / proximity
        fvg_hit = lows[i] <= zone["fvg_high"] * proximity and highs[i] >= zone["fvg_low"] / proximity
        if zone_hit or fvg_hit:
            return True
    return False


def trigger_score_1m(signal):
    """درجة الزخم من 30: MACD ميل صاعد + RSI صاعد + حجم فوق متوسطه."""
    score = 0
    if signal and signal.get("hist") > signal.get("hist_prev"):
        score += 10
    if signal and signal.get("rsi") > signal.get("rsi_prev"):
        score += 10
    if signal and signal.get("volume_ratio", 0.0) >= 1.0:
        score += 10
    return score


def dynamic_trailing_plan(symbol, price, atr_1m):
    """خطة تتبع ديناميكية بالكامل من ATR(14) لفريم 1m.

    activation_price = entry + 1.5 * ATR
    trailing_distance = 1.2 * ATR
    Binance يستقبل المسافة كنقاط BIPS؛ لذلك نحول المسافة السعرية إلى
    نسبة من السعر الحالي ثم إلى bips، مع احترام حدود TRAILING_DELTA للزوج.
    """
    price = float(price)
    atr_1m = float(atr_1m)
...
def build_candidate(symbol, frames, leader_frames, quote_volume):
    if not frames or quote_volume < MIN_24H_QUOTE_VOLUME:
        return None
    if any(not frames[x]["quality"]["ok"] for x in ("1m", "5m", "15m")):
        return None
    ...
    if not touched:
        ...
        return {"symbol": symbol, "zone": zone, "frames": frames, "leader_frames": leader_frames,
                "macro_reason": macro_reason, "confirmed": False, "trigger_score": 0, "quote_volume": quote_volume}
    book = book_stats(symbol)
    if BINANCE_ENV != "testnet" and (book["spread_pct"] > MAX_SPREAD_PCT or book["imbalance"] < MIN_BOOK_IMBALANCE):
        return None
    # الزناد موزون من 30 نقطة؛ لا يشترط تحقق المؤشرات الثلاثة معاً.
    trigger_score = trigger_score_1m(signal)
    confirmed = trigger_score >= 20
    score = 0.0
    score += 22.0  # العملة فوق EMA200 على 5m
    score += 20.0 if zone["green_count"] >= MIN_GREEN_CANDLES else 0.0
    score += 18.0 if zone["bos"] else 0.0
    score += 15.0 if zone["fvg_high"] > zone["fvg_low"] else 0.0
    score += 5.0 if zone.get("fib_ratio") else 0.0
    score += trigger_score / 3.0
    ...
    if not confirmed or score < MIN_SCORE:
        return {"symbol": symbol, "zone": zone, "frames": frames, "leader_frames": leader_frames,
                ...
                "book": book, "score": score, "trigger_score": trigger_score}
    price = book["ask"]
    ...
    if risk <= 0 or reward <= 0 or reward / risk < TAKE_RR_MIN:
        return None
    trailing = dynamic_trailing_plan(symbol, price, signal["atr"])
    if not trailing:
        return None
    ...
    return {
        ...
        "macro_reason": macro_reason, "confirmed": True, "trigger_score": trigger_score, "quote_volume": quote_volume,
        "signal": signal, "book": book, "price": price, "score": score,
        "stop": stop, "target": target, "stop_pct": stop_pct, "target_pct": target_pct,
        "atr_1m": trailing["atr_1m"], "activation_distance": trailing["activation_distance"],
        "trailing_distance": trailing["trailing_distance"], "activation_price": trailing["activation_price"],
        "raw_trail_bips": trailing["raw_trail_bips"], "trail_pct": trailing["trail_pct"],
        "trail_bips": trailing["trail_bips"], "created_at": time.time(),
    }
...
def ai_gate(candidate, news):
    ...
    prompt = (
        ...
        f"BOS=True, 5m فوق EMA200، spread={candidate['book']['spread_pct']:.3f}%.\n"
        f"الأخبار:\n{headlines}\n"
...
def main_keyboard():
    return [
        ...
        [button("🧯 إلغاء الأوامر", "cancelall"), button("📊 تقييم العملات", "rank")],
...
def status_text():
    ...
    lines = [
        ...
        f"✅ الناجحة: {m.get('wins', 0)} | 🔴 الخاسرة: {m.get('losses', 0)} | Win Rate: {(m.get('wins', 0) / max(1, m.get('wins', 0) + m.get('losses', 0)) * 100):.1f}%",
...
HELP_TEXT = (
    ...
    "/cancelall إلغاء أوامر البوت فقط\n/panic إلغاء الأوامر ومحاولة بيع مراكز البوت\n"
    "/coin SOL تحليل فني تفاعلي\n/chart BTC إرسال الشارت\n/prices أسعار السوق\n/rank تقييم أفضل العملات\n/ask سؤالك إلى Gemini\n"
    "/kill إنشاء قاطع تداول\n/unkill إزالة القاطع\n\n"
    "الدخول لا يحدث إلا بعد أن تكون العملة فوق EMA200 على 5m، "
    "ثم شمعتان خضراوان أو أكثر + BOS أو Order Block/FVG، وPullback، وTrigger Score 20/30 على 1m."
)
...
def setup_alert(candidate):
    ...
    return (
        ...
        f"• SMC {z.get('smc_type', 'FVG')}: <code>{fmt_price(candidate['symbol'], z['fvg_low'])}</code> — <code>{fmt_price(candidate['symbol'], z['fvg_high'])}</code>\n"
...
def entry_message(candidate, position=None, ai_text=""):
    ...
    lines = [
        ...
        "",
        "🏗 <b>سبب الدخول:</b>",
        f"• 5m: {z['green_count']} شموع خضراء + BOS + {z.get('smc_type', 'SMC')}",
        f"• Demand: <code>{fmt_price(symbol, z['zone_low'])}</code>-<code>{fmt_price(symbol, z['zone_high'])}</code>",
        f"• SMC {z.get('smc_type', 'FVG')}: <code>{fmt_price(symbol, z['fvg_low'])}</code>-<code>{fmt_price(symbol, z['fvg_high'])}</code>",
        f"• Fibonacci: <b>{z.get('fib_ratio') or '—'}</b> عند <code>{fmt_price(symbol, z.get('fib_price'))}</code>",
        f"• Pullback ✅ | Trigger Score: <b>{candidate.get('trigger_score', trigger_score_1m(signal))}/30</b> | MACD Hist <code>{signal['hist']:.8g}</code> | Volume <b>{signal['volume_ratio']:.2f}x</b>",
        f"• Macro ✅ {candidate['macro_reason']}",
        "",
        ...
        f"🛤 ATR(1m): <b>{candidate.get('atr_1m', signal['atr']):.8g}</b> | "
        f"التفعيل عند Entry + {TRAIL_ACTIVATION_ATR_MULT:.2f}×ATR = <code>{fmt_price(symbol, position.get('activation_price', candidate.get('activation_price')))}</code>",
        f"📏 مسافة Trailing: {TRAIL_DISTANCE_ATR_MULT:.2f}×ATR = <b>{candidate.get('trailing_distance', signal['atr'] * TRAIL_DISTANCE_ATR_MULT):.8g}</b> | "
        f"<b>{candidate['trail_pct']:.4f}% ({candidate['trail_bips']} bips)</b>",
...
def recover_oco_by_client_id(client_order_id):
    """يمنع تكرار البيع إذا قُبل OCO ثم ضاعت استجابة الشبكة."""
...
def place_initial_oco(symbol, qty, entry, stop_price, target_price):
    """OCO حديث، مع fallback صريح للواجهة القديمة عند رفض payload فقط.

    /api/v3/orderList/oco الحديثة تستخدم above*/below*.
    /api/v3/order/oco القديمة تستخدم price/stopPrice/stopLimitPrice.
    لا نخلط المعلمات بين النقطتين.
    """
    ...
    try:
        ...
        # إذا ضاعت الاستجابة بعد قبول الطلب، نتحقق من client id قبل أي بيع طارئ.
        ...
        # لا نعيد الطلب بعد timeout/شبكة؛ نستخدم القديم فقط عند رفض المعلمات الصريح.
        if not OCO_LEGACY_FALLBACK or modern_error.code not in (-1102, -1128, -2010):
            raise
...
def execute_entry(candidate, ai_text=""):
    ...
    try:
        ...
        with ENTRY_LOCK:
            other_reserved = max(Decimal("0"), RESERVED_QUOTE - ENTRY_RESERVATIONS.get(symbol, Decimal("0")))
        if free_quote < quote * D("1.01") + other_reserved or quote < rule["min_notional"]:
        ...
        position = {
            ...
            "atr_1m": float(candidate.get("atr_1m", candidate["frames"]["1m"]["atr"])),
            "activation_distance": float(candidate.get("activation_distance", 0.0)),
            "trailing_distance": float(candidate.get("trailing_distance", 0.0)),
            "activation_price": float(entry) + TRAIL_ACTIVATION_ATR_MULT * float(candidate.get("atr_1m", candidate["frames"]["1m"]["atr"])),
...
def activate_trailing(symbol, position, current_price):
    ...
    try:
        ...
        save_state()
        send_message(f"🛤 <b>تم تفعيل الوقف المتحرك — {symbol_name(symbol)}</b>\nالسعر: <code>{fmt_price(symbol, current_price)}</code> تجاوز مستوى ATR: <code>{fmt_price(symbol, position.get('activation_price'))}</code>\nATR(1m): <b>{position.get('atr_1m', 0):.8g}</b> | المسافة: <b>{position.get('trailing_distance', 0):.8g}</b>\nTrailing Delta: <b>{position['trail_bips']} bips</b>\nتم استبدال الحماية الثابتة بحماية متحركة على Binance.")
        return True
    except BinanceError as exc:
...
def monitor_position(symbol, position):
    ...
    if activation_price is None or float(position.get("atr_1m", 0.0)) <= 0:
        # مراكز الذاكرة القديمة تُهاجر إلى ATR حي بدلاً من استخدام نسبة ثابتة.
        try:
            live_frame = fetch_frame_cached(symbol, "1m", force=True)
            if live_frame and live_frame.get("atr", 0.0) > 0:
                position["atr_1m"] = float(live_frame["atr"])
                position["activation_distance"] = TRAIL_ACTIVATION_ATR_MULT * float(live_frame["atr"])
                position["trailing_distance"] = TRAIL_DISTANCE_ATR_MULT * float(live_frame["atr"])
                activation_price = entry + position["activation_distance"]
                position["activation_price"] = activation_price
                dynamic = dynamic_trailing_plan(symbol, entry, float(live_frame["atr"]))
                if dynamic:
                    position["trail_bips"] = dynamic["trail_bips"]
        except Exception as exc:
            log(f"تعذر تحديث ATR للمركز {symbol}: {exc}")
            activation_price = None
    if not position.get("trailing_active") and activation_price is not None and current >= float(activation_price):
...
def cancel_all_orders():
    """يلغي أوامر البوت المسجلة فقط، ولا يلغي أوامر المستخدم اليدوية."""
    for symbol, position in list(STATE.get("positions", {}).items()):
        try:
            if position.get("order_list_id"):
                CLIENT.cancel_oco(symbol, position["order_list_id"])
            else:
                order_ids = list(position.get("order_ids", []))
                if position.get("stop_order_id"):
                    order_ids.append(position["stop_order_id"])
                for order_id in set(order_ids):
                    if order_id is not None:
                        CLIENT.cancel_order(symbol, order_id)
        except BinanceError as exc:
            log(f"تعذر إلغاء حماية البوت لـ {symbol}: {exc}")
...
def reserve_entry(symbol):
    """حجز ذري للرمز وقيمة تقريبية قبل تشغيل خيط الدخول."""
    ...
    with ENTRY_LOCK:
        ...
        if not allowed:
            return False
        budget = D(min(TRADE_QUOTE_USD, MAX_POSITION_USD))
        ENTRY_RESERVATIONS[symbol] = budget
        RESERVED_QUOTE += budget
        return True
...
def release_entry(symbol):
    global RESERVED_QUOTE
    with ENTRY_LOCK:
        budget = ENTRY_RESERVATIONS.pop(symbol, Decimal("0"))
        RESERVED_QUOTE = max(Decimal("0"), RESERVED_QUOTE - budget)
...
def upsert_setup(candidate):
    ...
    setup = {
        ...
        "fvg_low": zone["fvg_low"], "fvg_high": zone["fvg_high"], "smc_type": zone.get("smc_type", "FVG"), "target_high": zone["target_high"],
...
def setup_as_candidate(candidate, setup):
    ...
    if risk <= 0 or reward <= 0 or reward / risk < TAKE_RR_MIN:
        return None
    signal = candidate["frames"]["1m"]
    trailing = dynamic_trailing_plan(candidate["symbol"], candidate["price"], signal["atr"])
    if not trailing:
        return None
    candidate.update({
        "stop_pct": risk / candidate["price"] * 100,
        "target_pct": reward / candidate["price"] * 100,
        "atr_1m": trailing["atr_1m"],
        "activation_distance": trailing["activation_distance"],
        "trailing_distance": trailing["trailing_distance"],
        "activation_price": trailing["activation_price"],
        "raw_trail_bips": trailing["raw_trail_bips"],
        "trail_pct": trailing["trail_pct"],
        "trail_bips": trailing["trail_bips"],
    })
...
def cleanup_expired_setups():
    """يحذف الإعدادات المنتهية بأمان من الذاكرة وواجهة الحالة."""
    now = time.time()
    removed = []
    with STATE_LOCK:
        for symbol, setup in list(STATE.get("setups", {}).items()):
            if now > float(setup.get("expires_at", 0)):
                STATE["setups"].pop(symbol, None)
                removed.append(symbol)
    if removed:
        log("حذف إعدادات منتهية: " + ", ".join(removed))
        save_state()
    return removed


def scan_one_symbol(symbol, tickers):
    """عامل فحص مستقل؛ لا يلمس STATE لتجنب تعارض الخيوط."""
    ...
    try:
        ...
        return build_candidate(symbol, frames, {}, quote_volume)
    except Exception as exc:
        log(f"خطأ تحليل {symbol}: {exc}")
        return None


def rank_one_symbol(symbol, tickers):
    """مؤشرات خفيفة للترتيب، دون طلب دفتر الأوامر."""
    if symbol not in SYMBOL_RULES or SYMBOL_RULES[symbol].get("status") != "TRADING":
        return None
    try:
        frames = fetch_frames(symbol)
        if not frames:
            return None
        macro_ok, macro_reason = macro_trend_ok(frames, {})
        zone = detect_demand_zone(frames["5m"]) if macro_ok else None
        touched = zone_touched(zone, frames["1m"]) if zone else False
        trigger = trigger_score_1m(frames["1m"])
        proximity = 0.0
        if zone:
            price = frames["1m"]["close"]
            center = (zone["zone_low"] + zone["zone_high"]) / 2.0
            proximity = max(0.0, 1.0 - abs(price - center) / max(price, 1e-12))
        rank_score = (35 if macro_ok else 0) + (30 if zone else 0) + (20 if touched else 0) + trigger + proximity * 5
        return {
            "symbol": symbol, "macro": "فوق EMA200" if macro_ok else "تحت EMA200",
            "zone": "نعم" if zone else "لا", "touched": touched,
            "trigger_score": trigger, "rank_score": rank_score,
        }
    except Exception as exc:
        log(f"خطأ ترتيب {symbol}: {exc}")
        return None


def rank_report():
    """تقييم أفضل 50 زوجاً وإرسال أفضل 10 قرباً من فرصة مكتملة."""
    refresh_universe(force=False)
    try:
        tickers = CLIENT.ticker_24h(SYMBOLS)
    except BinanceError as exc:
        return f"⚠️ تعذر جلب بيانات الترتيب: {esc(exc)}"
    results = []
    symbols = list(SYMBOLS)
    with ThreadPoolExecutor(max_workers=SCAN_WORKERS) as pool:
        futures = []
        for symbol in symbols:
            futures.append(pool.submit(rank_one_symbol, symbol, tickers))
            time.sleep(0.02)
    ...
    lines = [f"📊 <b>ترتيب فرص السكالبينج — {len(symbols)} زوجاً</b>", "Coin | Macro 5m | Demand | Trigger/30"]
    for i, item in enumerate(results[:10], 1):
        lines.append(f"{i}. {symbol_name(item['symbol'])} | {item['macro']} | {item['zone']} | {item['trigger_score']}/30")
    if len(lines) == 2:
        lines.append("لا توجد بيانات كافية حالياً؛ ستتم إعادة المحاولة.")
    return "\n".join(lines)
...
def run_scan(force=False):
    ...
    try:
        ...
        # لا توجد قراءة إلزامية لـ BTC؛ كل عملة تُحلّل من بياناتها الخاصة.
        tickers = CLIENT.ticker_24h(SYMBOLS)
        symbols = [x for x in list(SYMBOLS) if x not in STATE.get("positions", {}) and x not in ENTRY_RESERVATIONS]
        raw_candidates = []
        # 12 خيطاً كحد أقصى مع HTTP limiter=20؛ أسرع من التسلسل دون فتح 50 اتصالاً دفعة واحدة.
        with ThreadPoolExecutor(max_workers=SCAN_WORKERS) as pool:
            futures = []
            for symbol in symbols:
                futures.append(pool.submit(scan_one_symbol, symbol, tickers))
                time.sleep(0.02)
            for future in as_completed(futures):
...
def manual_analysis(symbol, chart_only=False, chat_id=None):
    try:
        ...
        frames = fetch_frames(symbol)
        if not frames:
            send_message(f"⚠️ بيانات {symbol_name(symbol)} غير مكتملة.", chat_id=chat_id)
            return
        candidate = None
        try:
        ...
        path = make_chart(candidate)
        if path:
            send_photo(path, f"{symbol_name(symbol)} | تحليل 1m/5m", chat_id)
        elif chart_only:
...
def ask_gemini(question, chat_id=None):
    if not GEMINI_API_KEY:
        ...
        return
    try:
        context = ["تحليل الدخول يعتمد على فريم 5m للعملة نفسها فقط"]
...
def handle_callback(update):
    ...
    if data == "status":
        ...
        send_message("🧯 جارٍ إلغاء أوامر البوت.", main_keyboard(), chat)
    elif data == "rank":
        threading.Thread(target=lambda: send_message(rank_report(), main_keyboard(), chat), daemon=True).start()
    elif data == "help":
...
def selftest():
    ...
    # اختبار صريح لمنطق الشمعتين + BOS + FVG.
    m = 100
    oo = [100.0] * m
    cc = [100.0] * m
    hh = [101.0] * m
    ...
    check("Demand Zone + 2 Green + BOS + FVG", zone is not None and zone["green_count"] >= MIN_GREEN_CANDLES and zone["bos"])
...
def main():
    ...
    send_message(
        ...
        f"أفضل {TOP_N_SYMBOLS} أزواج حسب حجم التداول 24س | BTC القائد\n"
