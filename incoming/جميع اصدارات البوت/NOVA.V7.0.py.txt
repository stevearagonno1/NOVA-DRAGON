...
# -*- coding: utf-8 -*-
"""
NOVA V7.0 UNIFIED — Hybrid + Scoring V5.3 + Shadow Lab + Maker + A/B + Governor
==============================================================================
...
V7.0 — النسخة الموحّدة (أقوى ما في الأرشيف في هيكل واحد):
11) زناد التنقيط المرجّح (روح V5.3 ELITE): نقاط الاتجاه (EMA50) + التدفق
    المتدرّج + كسر البنية MSS + الحجم + فجوة FVG + مكافأة الحيتان ⇒ قبول
    فوق SCORE_MIN_TOTAL بمكوّنين مستقلين على الأقل — بفلاتر V5.3 القاطعة
    (سوق ميت / مطاردة قمة / فوضى VR) + فيتو مزاج العملة (ذاكرة تعلم بسيطة).
12) التسليم المتتابع الشرطي: 3 صفقات/عملة/10د — الثانية والثالثة فقط إذا
    ربحت السابقة (لا تكديس خاسرات على نفس السعر).
13) مراجعة الأخوات كل 10د: الأضعف (ربحه ≤ 0) يخرج إذا كان هناك قوي رابح
    في نفس العملة — رأس المال ينتقل من الفاشل إلى الناجح.
14) الوضع المزدوج A/B: ساعات UTC الزوجية تيربو (5ث) والفردية هادئ (40ث) —
    كل صفقة موسومة بوضعها والتقارير تقارن صافي كل وضع مباشرة.
15) قاطع فوضى السوق التلقائي + تنبيه زحمة الفرص + تقارير تيليجرام آلية
    (دورية كل ساعة + يومية) + كون الدلاء (سيولة/متحركون/عمق).
...
STARTUP_TEXT = "✅ NOVA V7.0 UNIFIED AI-HFT Started & Connected to Binance WebSockets."
...
def shadow_open(signal):
    ...
    sid = f"SH#{symbol}@{trigger}@{int(now * 1000)}#{SHADOW_SEQ}"
    sh["open"][sid] = {
...
# ── V7.0: الوضع المزدوج A/B — نصف الساعات تيربو ونصفها هادئ ─────────────────

def trade_mode():
    """الوضع الحالي: ساعات UTC الزوجية = تيربو (5ث)، الفردية = هادئ (SPRAY_COOLDOWN)."""
    if not AB_MODE:
        return "calm"
    return "turbo" if (time.gmtime().tm_hour % 2 == 0) else "calm"


def mode_cooldown():
    """التهدئة الفعلية حسب وضع الساعة الحالي."""
    return TURBO_COOLDOWN_SEC if trade_mode() == "turbo" else SPRAY_COOLDOWN_SEC


# ── V7.0: قاطع فوضى السوق التلقائي ─────────────────────────────────────────

def chaos_blocked():
    """(ممنوع؟, سبب): تقلب BTC انفجاري أو انهيار حاد ⇒ تجميد دخولات مؤقت تلقائي.

    الحاكم يحمي من سلسلة خسائرك؛ هذا القاطع يحمي من جنون السوق نفسه.
    يُفعَّل بكسل (lazy) من بوابات الدخول ويُخزَّن في CHAOS_UNTIL.
    """
    global CHAOS_UNTIL
    if not CHAOS_BREAKER:
        return False, ""
    now = time.time()
    if now < CHAOS_UNTIL:
        return True, "chaos-pause"
    stats = btc_ctx_stats()
    vr = float(stats.get("vr", 1.0) or 1.0)
    drop = stats.get("drop_pct")
    trigger = ""
    if vr > CHAOS_VR_LIMIT:
        trigger = f"فوضى السوق: VR={vr:.2f} > {CHAOS_VR_LIMIT:g} ⇒ تجميد {CHAOS_PAUSE_SEC // 60}د"
    elif drop is not None and drop <= -CHAOS_DROP_LIMIT:
        trigger = (f"فوضى السوق: BTC {drop:.2f}% ≤ -{CHAOS_DROP_LIMIT:g}% "
                   f"⇒ تجميد {CHAOS_PAUSE_SEC // 60}د")
    if trigger:
        CHAOS_UNTIL = now + CHAOS_PAUSE_SEC
        FUNNEL["chaos-breaker"] += 1
        log(f"🌪️ {trigger}")
        return True, "chaos-pause"
    return False, ""


# ── V7.0: زحمة الفرص — لحظات انفجار الفرص لا تمر صامتة ──────────────────────

def crowd_register_and_check():
    """يسجّل إطلاقاً ويعيد نص التنبيه إن اكتملت الزحمة (وإلا None). دالة نقية الحساب."""
    global CROWD_LAST_ALERT
    now = time.monotonic()
    CROWD_TS.append(now)
    while CROWD_TS and now - CROWD_TS[0] > CROWD_WINDOW_SEC:
        CROWD_TS.popleft()
    if len(CROWD_TS) >= CROWD_COUNT and now - CROWD_LAST_ALERT > CROWD_COOLDOWN_SEC:
        CROWD_LAST_ALERT = now
        CROWD_TS.clear()
        FUNNEL["crowd-alert"] += 1
        return (f"🚨 زحمة فرص! {CROWD_COUNT}+ إطلاقات خلال {CROWD_WINDOW_SEC:.0f}ث — "
                f"السوق يتحرك الآن بحدة (تُراجع /status)")
    return None


# ── V7.0: مزاج العملات (Flavor Memory) — تعلّم بسيط لكل رمز ─────────────────

def normalize_flavor(raw):
    """معقم ذاكرة المزاج: أعداد صحيحة وأرباح عائمة أو إدخال نظيف فارغ."""
    ...
    for sym, f in raw.items():
        if not isinstance(f, dict):
            continue
        try:
            out[str(sym)] = {"n": int(f.get("n", 0) or 0),
                             "net": float(f.get("net", 0.0) or 0.0),
...
def flavor_update(symbol, pnl_net):
    """يحدّث ذاكرة المزاج بعد كل إغلاق (تُستعمل لاحقاً في فيتو التنقيط)."""
    if not symbol:
        return
    flavor = STATE.setdefault("flavor", {})
    try:
        entry = flavor.get(symbol) if isinstance(flavor.get(symbol), dict) else {}
        n = int(entry.get("n", 0) or 0)
        net = float(entry.get("net", 0.0) or 0.0)
        wins = int(entry.get("wins", 0) or 0)
    except (TypeError, ValueError):
        n, net, wins = 0, 0.0, 0
    flavor[symbol] = {"n": n + 1, "net": round(net + float(pnl_net), 6),
                      "wins": wins + (1 if pnl_net >= 0 else 0)}


def flavor_bad(symbol):
    """هل تاريخ هذه العملة سيئ بما يكفي لرفض إشارة تنقيط جديدة؟"""
    f = (STATE.get("flavor") or {}).get(symbol)
    if not isinstance(f, dict):
        return False
    try:
        ...
        return False
    return n >= SCORE_FLAVOR_MIN_TRADES and (net / n) < SCORE_FLAVOR_VETO_NET


# ── V7.0: التسليم المتتابع الشرطي — التركيز على الرابح بلا تكديس خاسرات ──────

def pyramid_allowed(symbol, price):
    """(مسموح؟, سبب): قاعدة 3/10د مع شرط الربح — الدخول 2 و3 فقط إذا ربحت السابقة."""
    if not PYRAMID_ON_PROFIT:
        return True, ""
    now = time.time()
    positions = [(pid, p) for pid, p in (STATE.get("positions") or {}).items()
                 if p.get("symbol") == symbol]
    if not positions:
        return True, ""
    # شرط الربح: مركز واحد على الأقل من الحاليين رابح بسعر السوق الحالي
    profitable = False
    window_count = 0
    for _pid, p in positions:
        entry = float(p.get("entry", 0) or 0)
        last = float(p.get("last_price", 0) or 0) or float(price or 0)
        if entry > 0 and last > 0 and last > entry:
            profitable = True
        opened = float(p.get("opened_at", 0) or 0)
        if now - opened <= PYRAMID_WINDOW_SEC:
            window_count += 1
    if window_count >= PYRAMID_WINDOW_MAX:
        FUNNEL["pyramid-window"] += 1
        return False, f"نافذة {PYRAMID_WINDOW_MAX}/10د ممتلئة"
    if not profitable:
        FUNNEL["pyramid-profit"] += 1
        return False, "لا مركز رابح على العملة (التسليم المتتابع يتطلب ربحاً سائداً)"
    return True, ""


# ── V7.0: مراجعة الأخوات — الضعيف يخرج والقوي يبقى (كل 10 دقائق) ─────────────

def sibling_review_once():
    """مقارنة مراكز نفس العملة: الأضعف (ربحه ≤ 0) يُصفّى إذا كان هناك قوي رابح.

    يعيد عدد المراكز المُصفّاة. الفكرة من صاحبها: رأس المال ينتقل من الفاشل
    إلى الناجح — ولا نلمس المجموعة كلها إذا كانت كلها خاسرة (ذلك عمل الوقف
    الزمني والحاكم، لا عمل المراجعة).
    """
    if not SIBLING_REVIEW_ENABLED:
        return 0
    groups = {}
    for pid, p in (STATE.get("positions") or {}).items():
        groups.setdefault(p.get("symbol", ""), []).append((pid, p))
    now = time.time()
    cut = 0
    for symbol, members in groups.items():
        ...
        for pid, p in members:
            ...
            if entry <= 0 or last <= 0:
                break
            if now - opened < SIBLING_MIN_AGE_SEC:
                break              # مركز طازج — يُعطى فرصته قبل الحكم عليه
            scored.append((pid, (last - entry) / entry * 100.0))
        else:
            if len(scored) < 2:
                continue
            scored.sort(key=lambda x: x[1])
            weakest_pid, weakest_gain = scored[0]
            strongest_gain = scored[-1][1]
            if weakest_gain <= 0.0 < strongest_gain:
                FUNNEL["sibling-cut"] += 1
                log(f"✂️ مراجعة الأخوات {weakest_pid}: الأضعف {weakest_gain:+.2f}% مقابل "
                    f"قوي {strongest_gain:+.2f}% — رأس المال ينتقل للناجح")
                position = (STATE.get("positions") or {}).get(weakest_pid)
                if position is not None:
                    asyncio.get_running_loop().create_task(force_exit_position(
                        weakest_pid, dict(position),
                        f"مراجعة الأخوات (ضعيف {weakest_gain:+.2f}% مقابل قوي "
                        f"{strongest_gain:+.2f}%)"))
                    cut += 1
    return cut


async def sibling_loop():
    """حلقة مراجعة الأخوات: كل SIBLING_REVIEW_EVERY دون أي REST إضافي."""
    global SIBLING_LAST
    while True:
        await asyncio.sleep(30.0)
        now = time.monotonic()
        if now - SIBLING_LAST < SIBLING_REVIEW_EVERY:
            continue
        SIBLING_LAST = now
        try:
            sibling_review_once()
            ...
            log(f"خطأ مراجعة الأخوات: {exc}")


# ── V7.0: زناد التنقيط المرجّح — روح V5.3 ELITE على شموع 1m حدثية ───────────

def _ema_last(values, period):
    """آخر قيمة EMA عبر pandas (موجودة أصلاً) — قيم غير كافية ⇒ 0.0."""
    try:
...
def scoring_evaluate(symbol):
    """تقييم تنقيط V5.3 المُكيَّف لـ1m: نقاط الاتجاه/التدفق/البنية/الحجم/الحيتان.

    يعيد None (لا إشارة) أو {"points", "components", "trend", "flow_pts",
    "mss", "vol_pts", "bonus", "close", "ema50"} — نقية وقابلة للاختبار.
    الفلاتر القاطعة: سوق ميت، مطاردة قمة، فوضى VR، مزاج عملة سيئ.
    """
    if not SCORE_TRIGGER_ENABLED:
        return None
    buffer = CANDLES.get((symbol, MICRO_INTERVAL))
    if not buffer:
        return None
    closes = [float(c) for c in (buffer.get("c") or []) if float(c) > 0]
    vols = [float(v) for v in (buffer.get("v") or [])]
    if len(closes) < SCORE_MIN_CANDLES:
        return None
    price, _age = market_price(symbol)
    if price <= 0:
        return None
    atr_1m = micro_atr(symbol)
    atr_pct = (atr_1m / price * 100.0) if atr_1m > 0 else 0.0
    if atr_pct < SCORE_DEAD_ATR_PCT:
        FUNNEL["score-dead"] += 1
        return None                          # سوق ميت لا يغطي التكاليف
    ema50 = _ema_last(closes, 50)
    if ema50 <= 0:
        return None
    if (price - ema50) / atr_1m > SCORE_CHASE_ATR if atr_1m > 0 else False:
        FUNNEL["score-chase"] += 1
        return None                          # مطاردة قمة ممتدة
    vr = float(btc_ctx_stats().get("vr", 1.0) or 1.0)
    if vr > SCORE_VR_EXTREME:
        FUNNEL["score-vr"] += 1
        return None                          # فوضى — لا تنقيط
    if flavor_bad(symbol):
        FUNNEL["score-flavor"] += 1
        return None                          # مزاج العملة سيئ تاريخياً
    # المكوّن 1 — الاتجاه: فوق EMA50 وميل صاعد (20) أو فوقه مسطح (10)
    slope_up = len(closes) > 52 and closes[-1] > closes[-8]
    if price > ema50 and slope_up:
        trend = SCORE_TREND_PTS
    elif price > ema50:
        trend = SCORE_TREND_FLAT_PTS
    else:
        trend = 0.0
    # المكوّن 2 — التدفق الحقيقي متدرّج (لا 0/كله)
    ratio, _fage = flow_imbalance(symbol)
    if ratio >= SPRAY_FLOW_RATIO:
        flow_pts = SCORE_FLOW_STRONG
    elif ratio >= SPRAY_IMPULSE_FLOW:
        flow_pts = SCORE_FLOW_MED
    elif ratio >= 1.3:
        flow_pts = SCORE_FLOW_WEAK
    else:
        flow_pts = 0.0
    # المكوّن 3 — MSS مصغّر: السعر يكسر قمة آخر N شمعة مغلقة
    window = closes[-SCORE_MSS_LOOKBACK:-1] if len(closes) > SCORE_MSS_LOOKBACK else closes[:-1]
    mss = 0.0
    if window and price > max(window):
        mss = SCORE_MSS_PTS
    # المكوّن 4 — الحجم: آخر شمعة مغلقة مقابل وسط 20 قبلها
    vol_pts = 0.0
    if len(vols) >= 21 and sum(vols[-21:-1]) > 0:
        ...
        if avg_vol > 0:
            r = vols[-1] / avg_vol
            if r >= 2.5:
                vol_pts = SCORE_VOL_HIGH_PTS
            elif r >= 1.5:
                vol_pts = SCORE_VOL_MED_PTS
    # مكافأة — إيقاظ حيتان حديث
    whale_hit, _usd = whale_recent(symbol, 60.0)
    bonus = SCORE_WHALE_BONUS if whale_hit else 0.0
    # فجوة صاعدة حديثة (FVG مصغّر على آخر 3 شموع مغلقة)
    fvg = 0.0
    if len(closes) >= 3 and len(buffer.get("l") or []) >= 3 and len(buffer.get("h") or []) >= 3:
        lows = [float(x) for x in buffer["l"][-3:]]
        highs = [float(x) for x in buffer["h"][-3:]]
        if lows[2] > highs[0] and lows[2] > lows[1]:
            fvg = SCORE_FVG_PTS
    components = sum(1 for x in (trend, flow_pts, mss, vol_pts) if x > 0)
    points = trend + flow_pts + mss + vol_pts + bonus + fvg
    if components < SCORE_MIN_COMPONENTS or points < SCORE_MIN_TOTAL:
        FUNNEL["score-low"] += 1
        return None
    return {"points": round(points, 1), "components": components, "trend": trend,
            "flow_pts": flow_pts, "mss": mss, "vol_pts": vol_pts, "bonus": bonus,
            "fvg": fvg, "close": float(price), "ema50": float(ema50), "vr": vr,
            "atr_1m": float(atr_1m)}


def scoring_build_signal(symbol, ev):
    """يبني إشارة التنقيط الجاهزة لمسار التنفيذ القياسي."""
    return {
        "symbol": symbol,
        "setup_id": f"{symbol}@score@{int(time.time() * 1000)}",
        "trigger": "score",
        "points": float(ev["points"]), "components": int(ev["components"]),
        "atr": 0.0, "atr_1m": float(ev["atr_1m"]), "vr": float(ev["vr"]),
        "close": float(ev["close"]), "live": float(ev["close"]),
        "score": float(ev["points"]),
        "flow_imbalance": 0.0, "created_at": time.time(),
    }


def scoring_on_candle_close(symbol, open_time):
    """يُستدعى من on_kline عند كل شمعة 1m مغلقة: تقييم واحد per شمعة per عملة.

    القياس الظلي مستمر دائماً (حتى في SHADOW_ONLY وأثناء الحاكم — عينات بلا
    فجوات مُحيّزة)، أما الإطلاق الحقيقي فيمر على بوابات المحرك والفوضى.
    """
    if not SCORE_TRIGGER_ENABLED:
        return
    if symbol in SCORE_LAST_T and SCORE_LAST_T[symbol] == open_time:
        return
    SCORE_LAST_T[symbol] = open_time
    now = time.monotonic()
    if now - float(SCORE_ATTEMPT.get(symbol, 0.0)) < 5.0:
        return
    SCORE_ATTEMPT[symbol] = now
    ev = scoring_evaluate(symbol)
    if ev is None:
        return
    signal = scoring_build_signal(symbol, ev)
    if signal["setup_id"] in SETUPS:
        return
    shadow_open(signal)                      # القياس بلا شروط تنفيذ
    if SHADOW_ONLY:
        FUNNEL["shadow-only"] += 1
        return
    if kill_switch_active() or STATE.get("paused") or governor_blocked()[0]:
        return
    if chaos_blocked()[0] or not _entry_gates_open(symbol):
        return
    FUNNEL["score-trigger"] += 1
    log(f"🎯 تنقيط {symbol}: {ev['points']:.0f} نقطة ({ev['components']} مكونات | "
        f"اتجاه {ev['trend']:.0f} تدفق {ev['flow_pts']:.0f} بنية {ev['mss']:.0f} "
        f"حجم {ev['vol_pts']:.0f}{' حوت +' + format(SCORE_WHALE_BONUS, 'g') if ev['bonus'] else ''}) → دخول")
    launch_entry(signal)


def hourly_report_text():
    """V7.0 — التقرير الدوري المختصر: من يحمل البوت ومن يخنقه الآن."""
    sh = (STATE.get("shadow") or {}).get("stats") or {}
    closed_total = sum(int(s.get("closed", 0) or 0) for s in sh.values())
    lines = [f"⏰ <b>تقرير دوري</b> — الوضع: {'⚡ تيربو' if trade_mode() == 'turbo' else '🐢 هادئ'} | "
             f"مراكز مفتوحة: {count_open_positions()}/{MAX_OPEN_POSITIONS}"]
    used, cap = order_budget_usage()
    gov_blocked, gov_why = governor_blocked()
    lines.append(f"أوامر: {used}/{cap} | الحاكم: {'🛑 ' + gov_why if gov_blocked else '🟢 حر'} | "
                 f"ظلي مغلق: {closed_total}")
    if sh:
        ranked = sorted(sh.items(), key=lambda kv: float(kv[1].get("net", 0) or 0), reverse=True)
        best_t, best_s = ranked[0]
        lines.append(f"أفضل زناد الآن: {best_t} ({float(best_s.get('net', 0) or 0):+.3f}$ صافي)")
    reasons = sorted(FUNNEL.items(), key=lambda kv: int(kv[1]), reverse=True)[:3]
    top = " | ".join(f"{k}: {v}" for k, v in reasons if int(v) > 0)
    if top:
        lines.append(f"أعلى الخناقات: {top}")
    return "\n".join(lines)


def daily_report_text():
    """V7.0 — تقرير نهاية اليوم: سطر يومية كامل من trade_log (تاريخ UTC)."""
    ...
    fees = sum(float(r.get("fees", 0) or 0) for r in rows)
    by_trigger = {}
    by_mode = {}
    for r in rows:
        t = str(r.get("trigger", "?") or "?")
        m = str(r.get("mode", "?") or "?")
        by_trigger[t] = by_trigger.get(t, 0.0) + float(r.get("pnl", 0) or 0)
        by_mode[m] = by_mode.get(m, 0.0) + float(r.get("pnl", 0) or 0)
    best = max(by_trigger.items(), key=lambda kv: kv[1]) if by_trigger else ("—", 0.0)
    worst = min(by_trigger.items(), key=lambda kv: kv[1]) if by_trigger else ("—", 0.0)
    lines = [f"📅 <b>يومية {today}</b>",
             f"صفقات: {n} | صافي: {net:+.4f}$ | رابحة: {wins}/{n} | رسوم: {fees:.4f}$"]
    if by_mode:
        modes = " | ".join(f"{m}: {v:+.3f}$" for m, v in sorted(by_mode.items()))
        lines.append(f"الأوضاع (A/B): {modes}")
    if by_trigger:
        lines.append(f"أفضل زناد: {best[0]} ({best[1]:+.3f}$) | أسوأ: {worst[0]} ({worst[1]:+.3f}$)")
    lines.append(f"بيع جزئي: {int(STATE.get('scale_outs', 0))} | "
                 f"رسوم تراكمية: {float(STATE.get('fees_paid', 0.0) or 0.0):.4f}$ | "
                 f"الحاكم (متتالية/ساعة/يوم): {FUNNEL.get('gov-streak', 0)}/"
                 f"{FUNNEL.get('gov-hourly', 0)}/{FUNNEL.get('gov-daily', 0)}")
    return "\n".join(lines)


async def insights_loop():
    """V7.0 — حلقة التقارير الآلية: دوري كل STATUS_REPORT_EVERY + يومية عند ساعتها."""
    global STATUS_LAST, DAILY_LAST
    while True:
        ...
        try:
            now = time.time()
            if now - STATUS_LAST >= STATUS_REPORT_EVERY:
                STATUS_LAST = now
                await send_message(hourly_report_text(), main_keyboard(),
                                   chat_id=TELEGRAM_CHAT_ID)
            if DAILY_REPORT_ENABLED:
                today = datetime.now(timezone.utc).strftime("%Y-%m-%d")
                if DAILY_LAST != today and time.gmtime().tm_hour == DAILY_REPORT_HOUR_UTC:
                    DAILY_LAST = today
                    await send_message(daily_report_text(), main_keyboard(),
                                       chat_id=TELEGRAM_CHAT_ID)
        except asyncio.CancelledError:
            raise
        except Exception as exc:
            log(f"خطأ حلقة التقارير: {exc}")
...
# ── V7.0: التسليم المتتابع الشرطي (3 صفقات/عملة/10د — 2 و3 فقط بعد ربح السابقة) ──
PYRAMID_ON_PROFIT = env_bool("PYRAMID_ON_PROFIT", True)
PYRAMID_WINDOW_SEC = clamp(env_float("PYRAMID_WINDOW_SEC", 600.0), 60.0, 3600.0)
PYRAMID_WINDOW_MAX = max(1, env_int("PYRAMID_WINDOW_MAX", 3))

# ── V7.0: مراجعة الأخوات — الضعيف يخرج والقوي يبقى (كل 10 دقائق) ────────────
SIBLING_REVIEW_ENABLED = env_bool("SIBLING_REVIEW_ENABLED", True)
SIBLING_REVIEW_EVERY = clamp(env_float("SIBLING_REVIEW_EVERY", 600.0), 60.0, 3600.0)
SIBLING_MIN_AGE_SEC = clamp(env_float("SIBLING_MIN_AGE_SEC", 60.0), 10.0, 900.0)

# ── V7.0: زناد التنقيط المرجّح (روح V5.3 ELITE على 1m حدثي) ─────────────────
SCORE_TRIGGER_ENABLED = env_bool("SCORE_TRIGGER_ENABLED", True)
SCORE_MIN_TOTAL = env_float("SCORE_MIN_TOTAL", 55.0)
SCORE_MIN_COMPONENTS = max(1, env_int("SCORE_MIN_COMPONENTS", 2))
SCORE_TREND_PTS = env_float("SCORE_TREND_PTS", 20.0)
SCORE_TREND_FLAT_PTS = env_float("SCORE_TREND_FLAT_PTS", 10.0)
SCORE_FLOW_STRONG = env_float("SCORE_FLOW_STRONG", 20.0)   # تدفق ≥ 2.5x
SCORE_FLOW_MED = env_float("SCORE_FLOW_MED", 14.0)         # ≥ 1.8x
SCORE_FLOW_WEAK = env_float("SCORE_FLOW_WEAK", 8.0)        # ≥ 1.3x
SCORE_MSS_PTS = env_float("SCORE_MSS_PTS", 20.0)           # كسر بنية علوية
SCORE_FVG_PTS = env_float("SCORE_FVG_PTS", 10.0)           # فجوة صاعدة حديثة
SCORE_VOL_MED_PTS = env_float("SCORE_VOL_MED_PTS", 10.0)   # حجم ≥ 1.5× الوسط
SCORE_VOL_HIGH_PTS = env_float("SCORE_VOL_HIGH_PTS", 15.0) # ≥ 2.5×
SCORE_WHALE_BONUS = env_float("SCORE_WHALE_BONUS", 25.0)  # إيقاظ حيتان
SCORE_MSS_LOOKBACK = max(4, env_int("SCORE_MSS_LOOKBACK", 12))
SCORE_DEAD_ATR_PCT = env_float("SCORE_DEAD_ATR_PCT", 0.08)  # سوق ميت: ATR% أدنى
SCORE_CHASE_ATR = env_float("SCORE_CHASE_ATR", 3.0)         # مطاردة قمة
SCORE_VR_EXTREME = env_float("SCORE_VR_EXTREME", 3.0)       # فوضى
SCORE_MIN_CANDLES = max(40, env_int("SCORE_MIN_CANDLES", 60))
SCORE_FLAVOR_MIN_TRADES = max(3, env_int("SCORE_FLAVOR_MIN_TRADES", 5))
SCORE_FLAVOR_VETO_NET = env_float("SCORE_FLAVOR_VETO_NET", -0.05)  # متوسط صفقة سيء

# ── V7.0: الوضع المزدوج A/B (ساعات زوجية تيربو، فردية هادئة) ────────────────
AB_MODE = env_bool("AB_MODE", True)
TURBO_COOLDOWN_SEC = clamp(env_float("TURBO_COOLDOWN_SEC", 5.0), 1.0, 60.0)
# SPRAY_COOLDOWN_SEC هو تهدئة الوضع الهادئ (40ث افتراضياً)

# ── V7.0: قاطع فوضى السوق التلقائي ─────────────────────────────────────────
CHAOS_BREAKER = env_bool("CHAOS_BREAKER", True)
CHAOS_VR_LIMIT = env_float("CHAOS_VR_LIMIT", 3.0)
CHAOS_DROP_LIMIT = env_float("CHAOS_DROP_LIMIT", 1.5)
CHAOS_PAUSE_SEC = clamp(env_float("CHAOS_PAUSE_SEC", 900.0), 300.0, 7200.0)

# ── V7.0: زحمة الفرص ────────────────────────────────────────────────────────
CROWD_WINDOW_SEC = clamp(env_float("CROWD_WINDOW_SEC", 60.0), 20.0, 300.0)
CROWD_COUNT = max(3, env_int("CROWD_COUNT", 5))
CROWD_COOLDOWN_SEC = clamp(env_float("CROWD_COOLDOWN_SEC", 600.0), 120.0, 3600.0)

# ── V7.0: التقارير الآلية (مصرّح بها صراحةً بتمرير chat_id) ─────────────────
STATUS_REPORT_EVERY = clamp(env_float("STATUS_REPORT_EVERY", 3600.0), 600.0, 86400.0)
DAILY_REPORT_ENABLED = env_bool("DAILY_REPORT_ENABLED", True)
DAILY_REPORT_HOUR_UTC = max(0, min(23, env_int("DAILY_REPORT_HOUR_UTC", 20)))

# ── V7.0: كون الدلاء (سيولة + تقلب + متحركون) ──────────────────────────────
UNIVERSE_MODE = os.getenv("UNIVERSE_MODE", "buckets").strip().lower()
if UNIVERSE_MODE not in ("buckets", "volume"):
    UNIVERSE_MODE = "buckets"
UNIVERSE_VOL_SHARE = clamp(env_float("UNIVERSE_VOL_SHARE", 0.6), 0.2, 1.0)
UNIVERSE_MOVE_SHARE = clamp(env_float("UNIVERSE_MOVE_SHARE", 0.25), 0.0, 0.6)
...
SCORE_ATTEMPT = {}    # V7.0: symbol -> ts — خنق تقييم زناد التنقيط (كل دقيقة)
SCORE_LAST_T = {}     # V7.0: symbol -> candle open time — كل شمعة مغلقة مرة واحدة
CHAOS_UNTIL = 0.0     # V7.0: طابع time.time — إيقاف فوضى السوق التلقائي
SHADOW_SEQ = 0        # V7.0: عدّاد تسلسلي — تفرد معرّف الصفقات الظلية حتى في نفس الملّي ثانية
CROWD_TS = deque()    # V7.0: طوابع الإطلاقات — كشف زحمة الفرص
# أختام monotonic تُهيّأ "في الماضي البعيد" — monotonic قد يبدأ صغيراً على بعض الأنظمة
CROWD_LAST_ALERT = time.monotonic() - (CROWD_COOLDOWN_SEC + 1.0)
SIBLING_LAST = time.monotonic() - (SIBLING_REVIEW_EVERY + 1.0)
STATUS_LAST = 0.0     # V7.0: آخر تقرير دوري
DAILY_LAST = ""       # V7.0: آخر يوم أُرسل له التقرير اليومي
...
def default_state():
    return {
        ...
        "flavor": {},            # V7.0: مزاج كل عملة {n, net, wins} — تعلّم بسيط
...
def load_state():
    ...
    try:
        ...
        base["flavor"] = normalize_flavor(base.get("flavor"))
...
def bucketize_universe(candidates, n):
    """V7.0 كون الدلاء: 60% عمق سيولة + 25% متحركون + 15% عمق إضافي — دالة نقية.

    candidates: [(quote_volume, abs_change_pct, symbol)] — يعيد حتى n رمزاً.
    """
    if not candidates or n <= 0:
        return []
    n = int(n)
    k_vol = max(1, int(round(n * UNIVERSE_VOL_SHARE)))
    k_move = max(0, int(round(n * UNIVERSE_MOVE_SHARE)))
    pool = sorted(candidates, key=lambda x: x[0], reverse=True)     # بالسيولة
    by_vol = [s for _v, _c, s in pool[:k_vol]]
    rest = [x for x in pool if x[2] not in set(by_vol)]
    by_move = [s for _v, _c, s in sorted(rest, key=lambda x: x[1], reverse=True)[:k_move]]
    rest2 = [x for x in rest if x[2] not in set(by_move)]
    by_depth = [s for _v, _c, s in rest2[:max(0, n - len(by_vol) - len(by_move))]]
    out = list(dict.fromkeys(by_vol + by_move + by_depth))
    return out[:n]
...
def pick_universe(tickers):
    ...
    for symbol, rule in SYMBOL_RULES.items():
        ...
        change = abs(float(row.get("change", 0.0) or 0.0))
        candidates.append((float(row.get("quote_volume", 0.0)), change, symbol))
    if UNIVERSE_MODE == "buckets":
        return bucketize_universe(candidates, max(1, TOP_N_SYMBOLS))
    candidates.sort(key=lambda x: x[0], reverse=True)
    return [symbol for _v, _c, symbol in candidates[: max(1, TOP_N_SYMBOLS)]]
...
async def refresh_universe():
    ...
    if chosen:
        ...
        log(f"كون V7 ({UNIVERSE_MODE}): أفضل {len(SYMBOLS)} زوجاً — دلاء 60% سيولة + 25% متحركون + عمق" if UNIVERSE_MODE == "buckets" else f"كون V7 ({UNIVERSE_MODE}): أفضل {len(SYMBOLS)} زوجاً حسب حجم 24س")
    return SYMBOLS
...
def _entry_gates_open(symbol):
    """بوابات رخيصة قبل أي تقييم: قاطع، حاكم، فوضى، سقف مراكز، تهدئة."""
    if kill_switch_active() or STATE.get("paused"):
        return False
...
def on_kline(data):
    ...
    try:
        ...
        # V7.0: زناد التنقيط المرجّح — تقييم واحد لكل شمعة مغلقة (حدثي بلا حجب)
        scoring_on_candle_close(symbol, int(k.get("t") or 0))
    except Exception as exc:
...
async def process_signal(signal):
    ...
    if veto:
        ...
        return False
    ok_pyr, why_pyr = pyramid_allowed(symbol, float(signal.get("price", 0) or 0))
    if not ok_pyr:
        log(f"إلغاء إشارة {symbol}: التسليم المتتابع — {why_pyr}")
        return False
    ...
    cooldown = None if is_sniper else mode_cooldown()   # V7.0: تهدئة وضع الساعة (A/B)
...
def launch_entry(signal):
    ...
    FUNNEL["fired"] += 1
    try:
        alert = crowd_register_and_check()
        if alert:
            asyncio.get_running_loop().create_task(
                send_message(alert, main_keyboard(), chat_id=TELEGRAM_CHAT_ID))
    except RuntimeError:
        pass
    try:
...
async def complete_entry(client_id, record, executed_qty, avg_price, quote_filled):
    ...
    try:
        ...
        position = {
            ...
            "mode": str(record.get("mode", trade_mode()) or "calm"),
...
def productivity_text():
    ...
    return (
        f"\n📊 إنتاجية V7.0 ({'⚠️ TESTNET' if BINANCE_ENV != 'live' else '🔴 LIVE'}):\n"
...
def context_text():
    ...
    return (
        ...
        + "\n".join(f"  • {name_ar}: {count}" for name_ar, count in (
            ...
            ("🎯 تنقيط V5.3 (إشارات)", FUNNEL.get("score-trigger", 0)),
            ("تنقيط: سوق ميت", FUNNEL.get("score-dead", 0)),
            ("تنقيط: مطاردة قمة", FUNNEL.get("score-chase", 0)),
            ("تنقيط: فوضى VR", FUNNEL.get("score-vr", 0)),
            ("تنقيط: نقاط دون العتبة", FUNNEL.get("score-low", 0)),
            ("تنقيط: مزاج عملة سيئ", FUNNEL.get("score-flavor", 0)),
            ("تسليم: نافذة 3/10د ممتلئة", FUNNEL.get("pyramid-window", 0)),
            ("تسليم: بلا رابح سائد", FUNNEL.get("pyramid-profit", 0)),
            ("✂️ مراجعة الأخوات (خروج)", FUNNEL.get("sibling-cut", 0)),
            ("🌪️ قاطع فوضى السوق", FUNNEL.get("chaos-breaker", 0)),
            ("🚨 زحمة فرص (تنبيه)", FUNNEL.get("crowd-alert", 0)),
            ("حجز: فوضى", FUNNEL.get("reserve-chaos", 0)),
...
HELP_TEXT = (
    "🤖 <b>NOVA V7.0 UNIFIED AI-HFT — رشاش + قنّاص + تنقيط + مختبر ظلي</b>\n\n"
...
def gemini_configure():
    ...
    if not GEMINI_API_KEY:
        ...
        return False
    try:
        genai.configure(api_key=__REDACTED__
        GEMINI_READY = True
...
async def handle_command(text, chat):
    ...
    if command in ("/start", "/menu"):
        await send_message("🎛️ <b>NOVA V7.0 UNIFIED AI-HFT</b>\nاختر عملية:", main_keyboard(), chat)
    elif command == "/help":
        await send_message(HELP_TEXT, main_keyboard(), chat)
    elif command == "/status":
        await send_message(status_text(), main_keyboard(), chat)
        ...
        try:
            ...
            if removed:
                ...
                for symbol in removed:
                    ...
                    SCORE_LAST_T.pop(symbol, None)     # V7.0.1: لا تراكم ذاكرة التنقيط
                    SCORE_ATTEMPT.pop(symbol, None)
                    if symbol in held:
...
async def run_selftest():
    ...
    print("NOVA V7.0 UNIFIED SELFTEST — Spray+Sniper / Fee-Aware Exits / Governor")
    results = []
    ...
    check("محرك القنّاص والدماغ الخطين مركّبان بالكامل",
          all(name in globals() for name in (
              ...
              "scoring_evaluate", "pyramid_allowed", "sibling_review_once", "chaos_blocked",
    ...
    # ═══ V7.0: النسخة الموحّدة ═══
    # 21) ثوابت V7.0 الحرفية
    check("V7: ثوابت التنقيط (قبول 55 نقطة + مكوّنان + فلاتر ميت/مطاردة/فوضى/مزاج)",
          SCORE_MIN_TOTAL == 55 and SCORE_MIN_COMPONENTS == 2 and SCORE_MIN_CANDLES == 60
          and SCORE_DEAD_ATR_PCT == 0.08 and SCORE_CHASE_ATR == 3.0 and SCORE_VR_EXTREME == 3.0
          and SCORE_FLAVOR_MIN_TRADES == 5 and SCORE_FLAVOR_VETO_NET == -0.05)
    check("V7: التسليم المتتابع (3/10د بشرط ربح) + الأخوات (كل 10د، عمر أدنى 60ث)",
          PYRAMID_ON_PROFIT and PYRAMID_WINDOW_SEC == 600 and PYRAMID_WINDOW_MAX == 3
          and SIBLING_REVIEW_ENABLED and SIBLING_REVIEW_EVERY == 600 and SIBLING_MIN_AGE_SEC == 60)
    check("V7: A/B (تيربو 5ث) + قاطع الفوضى (VR>3 أو −1.5% ⇒ 900ث) + الزحمة (5 خلال 60ث)",
          AB_MODE and TURBO_COOLDOWN_SEC == 5 and CHAOS_BREAKER and CHAOS_VR_LIMIT == 3.0
          and CHAOS_DROP_LIMIT == 1.5 and CHAOS_PAUSE_SEC == 900
          and CROWD_COUNT == 5 and CROWD_WINDOW_SEC == 60 and CROWD_COOLDOWN_SEC == 600)
    check("V7: التقارير (دوري كل ساعة + يومية 20UTC) + كون الدلاء (60/25) + النسخة V7.0",
          STATUS_REPORT_EVERY == 3600 and DAILY_REPORT_ENABLED and DAILY_REPORT_HOUR_UTC == 20
          and UNIVERSE_MODE == "buckets" and UNIVERSE_VOL_SHARE == 0.60 and UNIVERSE_MOVE_SHARE == 0.25
          and "V7.0" in STARTUP_TEXT and "V6.2" not in STARTUP_TEXT
          and callable(sibling_loop) and callable(insights_loop) and callable(scoring_on_candle_close))

    # 22) A/B: وضع الساعة يحدد التهدئة
    saved_ab = AB_MODE
    try:
        globals()["AB_MODE"] = False
        check("V7: A/B معطّل ⇒ هادئ بتهدئة الرشاش الافتراضية",
              trade_mode() == "calm" and mode_cooldown() == SPRAY_COOLDOWN_SEC)
        globals()["AB_MODE"] = True
        check("V7: A/B مفعّل ⇒ تهدئة من {تيربو 5ث، هادئ 40ث} حسب زوجية ساعة UTC",
              trade_mode() in ("turbo", "calm")
              and mode_cooldown() in (TURBO_COOLDOWN_SEC, SPRAY_COOLDOWN_SEC))
    finally:
        globals()["AB_MODE"] = saved_ab

    # 23) مزاج العملات: تعقيم + تراكم + فيتو
    check("V7: معقم المزاج يرفض القمامة ويحوّل النصوص لأرقام",
          normalize_flavor(None) == {} and normalize_flavor("x") == {}
          and normalize_flavor({"ABC": {"n": "3", "net": "0.15", "wins": 2}, "BAD": "junk"})
          == {"ABC": {"n": 3, "net": 0.15, "wins": 2}})
    STATE["flavor"] = {}
    flavor_update("TSTUSDT", -0.10)
    flavor_update("TSTUSDT", -0.10)
    flavor_update("TSTUSDT", 0.20)
    f_flavor = (STATE.get("flavor") or {}).get("TSTUSDT") or {}
    check("V7: المزاج يتراكم (3 صفقات، صافي 0.0، رابحة 1)",
          f_flavor.get("n") == 3 and abs(f_flavor.get("net", 9.0)) < 1e-9 and f_flavor.get("wins") == 1)
    STATE["flavor"]["TSTUSDT"] = {"n": 5, "net": -0.5, "wins": 1}
    check("V7: فيتو المزاج — متوسط −0.10$ مع n≥5 ⇒ مرفوض (وعملة أخرى حرة)",
          flavor_bad("TSTUSDT") and not flavor_bad("UNKNOWNUSDT"))
    STATE["flavor"] = {}

    # 24) التسليم المتتابع الشرطي
    STATE["positions"] = {"P1": {"symbol": "TSTUSDT", "entry": 100.0, "last_price": 99.0,
                                 "opened_at": time.time() - 60}}
    ok_pyr, why_pyr = pyramid_allowed("TSTUSDT", 99.0)
    check("V7: تسليم مرفوض — لا مركز رابح على العملة (لا تكديس خاسرات)", not ok_pyr and "ربح" in why_pyr)
    STATE["positions"]["P1"]["last_price"] = 101.0
    ok_pyr, _ = pyramid_allowed("TSTUSDT", 101.0)
    check("V7: تسليم مسموح — السابق رابح والنافذة 1/3", ok_pyr)
    STATE["positions"]["P2"] = {"symbol": "TSTUSDT", "entry": 100.5, "last_price": 101.0,
                                "opened_at": time.time() - 30}
    STATE["positions"]["P3"] = {"symbol": "TSTUSDT", "entry": 100.8, "last_price": 101.0,
                                "opened_at": time.time() - 20}
    ok_pyr, why_pyr = pyramid_allowed("TSTUSDT", 101.0)
    check("V7: تسليم مرفوض — نافذة 3/10د ممتلئة", not ok_pyr and "ممتلئة" in why_pyr)
    ok_pyr, _ = pyramid_allowed("OTHERUSDT", 100.0)
    check("V7: عملة بلا مراكز ⇒ تسليم حر", ok_pyr)
    STATE["positions"] = {}

    # 25) مراجعة الأخوات: الأضعف يخرج إذا وُجد قوي رابح
    now_ts = time.time()
    killed = []

    async def _fake_exit(pid, position, reason):
        killed.append(pid)

    _orig_exit = globals()["force_exit_position"]
    globals()["force_exit_position"] = _fake_exit
    try:
        STATE["positions"] = {
            "PS": {"symbol": "SBUSDT", "entry": 100.0, "last_price": 100.5, "opened_at": now_ts - 300},
            "PW": {"symbol": "SBUSDT", "entry": 100.0, "last_price": 99.0, "opened_at": now_ts - 300},
        }
        cut = sibling_review_once()
        await asyncio.sleep(0.05)                # مهلة لمهمة الإعدام المجدولة
        check("V7: الأخوات — الأضعف (−1%) يُصفّى والقوي (+0.5%) يبقى",
              killed == ["PW"] and cut == 1
              and "PS" in STATE["positions"] and "PW" in STATE["positions"])
        killed.clear()
        STATE["positions"] = {
            "PL1": {"symbol": "SB2USDT", "entry": 100.0, "last_price": 99.5, "opened_at": now_ts - 300},
            "PL2": {"symbol": "SB2USDT", "entry": 100.0, "last_price": 99.8, "opened_at": now_ts - 300},
        }
        check("V7: الأخوات — الجميع خاسر ⇒ لا مساس (ذلك عمل الوقف الزمني)", sibling_review_once() == 0)
        STATE["positions"] = {
            "PF": {"symbol": "SB3USDT", "entry": 100.0, "last_price": 99.0, "opened_at": now_ts - 10},
            "PO": {"symbol": "SB3USDT", "entry": 100.0, "last_price": 100.5, "opened_at": now_ts - 300},
        }
        check("V7: الأخوات — الطازج (<60ث) يعطى فرصته قبل الحكم", sibling_review_once() == 0)
    finally:
        globals()["force_exit_position"] = _orig_exit
    STATE["positions"] = {}

    # 26) التنقيط المرجّح: قبول الاتجاه القوي + فيتو الميت/المطاردة/الفوضى/المزاج
    def _feed_1m(sym, closes_in, vols_in=None):
        buf = {"t": [], "o": [], "h": [], "l": [], "c": [], "v": [], "live": 0.0}
        base = int(time.time() * 1000) // 60000 * 60000
        total = len(closes_in)
        for i, c_val in enumerate(closes_in):
            t_val = base - (total - i) * 60_000
            o_val = closes_in[i - 1] if i else c_val
            buf["t"].append(t_val)
            buf["o"].append(o_val)
            buf["h"].append(max(o_val, c_val) * 1.001)
            buf["l"].append(min(o_val, c_val) * 0.999)
            buf["c"].append(c_val)
            buf["v"].append(float(vols_in[i]) if vols_in else 1000.0)
        CANDLES[(sym, "1m")] = buf
        return buf

    _orig_mp, _orig_fi = globals()["market_price"], globals()["flow_imbalance"]
    _orig_wh, _orig_btc = globals()["whale_recent"], globals()["btc_ctx_stats"]
    _orig_matr = globals()["micro_atr"]
    try:
        globals()["micro_atr"] = lambda s, atr_fallback=0.0: 1.2   # ATR تركيبي واقعي
        globals()["market_price"] = lambda s: (110.0, 0.0)
        globals()["flow_imbalance"] = lambda s, max_age=None: (2.6, 0.0)
        globals()["whale_recent"] = lambda s, max_age=None: (False, 0.0)
        globals()["btc_ctx_stats"] = lambda: {"vr": 1.2, "drop_pct": -0.2}
        closes_up = [100.0 + i * (10.0 / 89) for i in range(90)]
        closes_up[-1] = 111.0
        _feed_1m("SCOREUSDT", closes_up)
        CANDLES[("SCOREUSDT", "1m")]["o"][-1] = 111.5   # فتح فوق قمة السابقة = فجوة
        CANDLES[("SCOREUSDT", "1m")]["l"][-1] = 111.4
        ev = scoring_evaluate("SCOREUSDT")
        check("V7: تنقيط — اتجاه قوي + تدفق 2.6x + كسر قمة + فجوة ⇒ قبول ≥55 بمكوّنين+",
              ev is not None and ev["points"] >= SCORE_MIN_TOTAL
              and ev["components"] >= SCORE_MIN_COMPONENTS and ev["trend"] == SCORE_TREND_PTS
              and ev["fvg"] == SCORE_FVG_PTS)
        _feed_1m("FLATUSDT", [100.0] * 90)
        globals()["market_price"] = lambda s: (100.0, 0.0)
        globals()["flow_imbalance"] = lambda s, max_age=None: (1.0, 0.0)
        check("V7: تنقيط — سوق مسطّح بلا تدفق ⇒ رفض (دون العتبة)",
              scoring_evaluate("FLATUSDT") is None and FUNNEL["score-low"] > 0)
        globals()["market_price"] = lambda s: (130.0, 0.0)     # ممتدة 3×ATR فوق EMA50
        check("V7: تنقيط — مطاردة قمة (ممتدة >3×ATR فوق EMA50) ⇒ رفض",
              scoring_evaluate("SCOREUSDT") is None and FUNNEL["score-chase"] > 0)
        globals()["market_price"] = lambda s: (110.0, 0.0)
        globals()["micro_atr"] = lambda s, atr_fallback=0.0: 0.05   # 0.045% ميت
        check("V7: تنقيط — سوق ميت (ATR% دون 0.08) ⇒ رفض",
              scoring_evaluate("SCOREUSDT") is None and FUNNEL["score-dead"] > 0)
        globals()["micro_atr"] = lambda s, atr_fallback=0.0: 1.2
        globals()["btc_ctx_stats"] = lambda: {"vr": 3.5, "drop_pct": -0.2}
        check("V7: تنقيط — فوضى VR>3 ⇒ رفض",
              scoring_evaluate("SCOREUSDT") is None and FUNNEL["score-vr"] > 0)
        globals()["btc_ctx_stats"] = lambda: {"vr": 1.2, "drop_pct": -0.2}
        STATE["flavor"]["SCOREUSDT"] = {"n": 6, "net": -0.6, "wins": 1}
        check("V7: تنقيط — مزاج عملة سيئ (متوسط −0.10$) ⇒ رفض",
              scoring_evaluate("SCOREUSDT") is None and FUNNEL["score-flavor"] > 0)
        STATE["flavor"] = {}
        globals()["whale_recent"] = lambda s, max_age=None: (True, 500.0)
        ev_whale = scoring_evaluate("SCOREUSDT")
        check("V7: تنقيط — مكافأة الحيتان +25 تُضاف للنقاط",
              ev_whale is not None and ev_whale["bonus"] == SCORE_WHALE_BONUS)

        # 27) زناد التنقيط: قياس ظلي دائم + إطلاق محجوب بالإيقاف + one-shot لكل شمعة
        SCORE_LAST_T.clear(); SCORE_ATTEMPT.clear()
        SHADOW_THROTTLE.clear()
        base_opened = int(((shadow_state().get("stats") or {}).get("score") or {}).get("opened", 0))
        b_trig = int(FUNNEL.get("score-trigger", 0))
        STATE["paused"] = True
        scoring_on_candle_close("SCOREUSDT", 777_000)
        after_first = int(((shadow_state().get("stats") or {}).get("score") or {}).get("opened", 0))
        scoring_on_candle_close("SCOREUSDT", 777_000)          # نفس الشمعة ⇒ لا شيء
        after_dup = int(((shadow_state().get("stats") or {}).get("score") or {}).get("opened", 0))
        check("V7: الزناد one-shot لكل شمعة + القياس الظلي يستمر أثناء الإيقاف",
              after_first == base_opened + 1 and after_dup == after_first
              and int(FUNNEL.get("score-trigger", 0)) == b_trig)
        STATE["paused"] = False
    finally:
        globals()["market_price"], globals()["flow_imbalance"] = _orig_mp, _orig_fi
        globals()["whale_recent"], globals()["btc_ctx_stats"] = _orig_wh, _orig_btc
        globals()["micro_atr"] = _orig_matr
    CANDLES.pop(("SCOREUSDT", "1m"), None)
    CANDLES.pop(("FLATUSDT", "1m"), None)

    # 28) قاطع فوضى السوق التلقائي
    globals()["CHAOS_UNTIL"] = 0.0
    _orig_btc2 = globals()["btc_ctx_stats"]
    try:
        globals()["btc_ctx_stats"] = lambda: {"vr": 3.5, "drop_pct": None}
        blocked, why_ch = chaos_blocked()
        check("V7: الفوضى — VR انفجاري ⇒ تجميد دخولات فوري", blocked and why_ch == "chaos-pause")
        check("V7: التجميد يبقى مفعلاً حتى نهاية المهلة (بلا إعادة حساب)",
              chaos_blocked()[0] and CHAOS_UNTIL > time.time())
        globals()["CHAOS_UNTIL"] = 0.0
        globals()["btc_ctx_stats"] = lambda: {"vr": 1.1, "drop_pct": -1.6}
        check("V7: الفوضى — انهيار BTC −1.6% ⇒ تجميد", chaos_blocked()[0])
        globals()["CHAOS_UNTIL"] = 0.0
        globals()["btc_ctx_stats"] = lambda: {"vr": 1.1, "drop_pct": -0.5}
        check("V7: سوق هادئ ⇒ لا تجميد", not chaos_blocked()[0])
    finally:
        globals()["btc_ctx_stats"] = _orig_btc2
        globals()["CHAOS_UNTIL"] = 0.0

    # 29) زحمة الفرص
    CROWD_TS.clear()
    FUNNEL["crowd-alert"] = 0
    globals()["CROWD_LAST_ALERT"] = time.monotonic() - (CROWD_COOLDOWN_SEC + 1.0)
    quiet = [crowd_register_and_check() for _ in range(4)]
    fifth = crowd_register_and_check()
    sixth = crowd_register_and_check()
    check("V7: الزحمة — 4 إطلاقات صامتة ثم تنبيه عند الخامسة ثم كتم فوري",
          all(q is None for q in quiet) and isinstance(fifth, str) and "زحمة" in fifth
          and sixth is None and FUNNEL["crowd-alert"] == 1)

    # 30) كون الدلاء: 3 سيولة + 1 متحرك + 1 عمق
    cands = [(1000.0, 1.0, "A"), (900.0, 5.0, "B"), (800.0, 2.0, "C"), (700.0, 9.0, "D"),
             (600.0, 3.0, "E"), (500.0, 4.0, "F"), (400.0, 8.0, "G"), (300.0, 6.0, "H"),
             (200.0, 7.0, "I"), (100.0, 0.5, "J")]
    check("V7: كون الدلاء — 3 سيولة + أعلى متحرك من الباقي + عمق، بلا تكرار",
          bucketize_universe(cands, 5) == ["A", "B", "C", "D", "E"])
    check("V7: كون الدلاء — مدخلات فارغة/صفرية ⇒ []",
          bucketize_universe([], 5) == [] and bucketize_universe(cands, 0) == [])

    # 31) التقارير الآلية: دوري + يومية (صفقات اليوم فقط وتفصيل A/B)
    today_str = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    STATE["trade_log"] = [
        {"ts": today_str + "T10:00:00+00:00", "symbol": "TSTUSDT", "pnl": 0.10,
         "trigger": "score", "fees": 0.01, "mode": "turbo"},
        {"ts": today_str + "T11:00:00+00:00", "symbol": "ABCUSDT", "pnl": -0.05,
         "trigger": "impulse", "fees": 0.01, "mode": "calm"},
        {"ts": "1999-01-01T00:00:00+00:00", "symbol": "OLDUSDT", "pnl": 99.0,
         "trigger": "x", "fees": 0.0, "mode": "calm"},
    ]
    hourly_txt = hourly_report_text()
    daily_txt = daily_report_text()
    check("V7: التقرير الدوري — وضع A/B + مراكز + حاكم + أوامر",
          "تقرير دوري" in hourly_txt and "الحاكم" in hourly_txt and "أوامر" in hourly_txt)
    check("V7: اليومية — صفقات اليوم فقط (2) وصافي +0.05 وتفصيل الأوضاع turbo/calm",
          "يومية" in daily_txt and "صفقات: 2" in daily_txt and "+0.0500$" in daily_txt
          and "turbo" in daily_txt and "calm" in daily_txt and "OLDUSDT" not in daily_txt)
    STATE["trade_log"] = []

    # 32) V7.0.1 تدقيق: مفصل on_kline → التنقيط (مغلقة فقط) + الحالات الحدّية
    _orig_syms, _orig_set = globals()["SYMBOLS"], globals()["SYMBOLS_SET"]
    _saved = {n: globals()[n] for n in ("market_price", "flow_imbalance", "whale_recent",
                                        "btc_ctx_stats", "micro_atr")}
    globals()["SYMBOLS"], globals()["SYMBOLS_SET"] = [], set()
    try:
        CANDLES.pop(("WIREUSDT", "1m"), None)
        SHADOW_THROTTLE.clear()
        SCORE_LAST_T.clear(); SCORE_ATTEMPT.clear()
        globals()["market_price"] = lambda s: (110.9, 0.0)
        globals()["flow_imbalance"] = lambda s, max_age=None: (2.6, 0.0)
        globals()["whale_recent"] = lambda s, max_age=None: (False, 0.0)
        globals()["btc_ctx_stats"] = lambda: {"vr": 1.2, "drop_pct": -0.2}
        globals()["micro_atr"] = lambda s, atr_fallback=0.0: 1.2
        closes_up = [100.0 + i * (10.0 / 89) for i in range(90)]
        _feed_1m("WIREUSDT", closes_up)
        buf_wire = CANDLES[("WIREUSDT", "1m")]
        n_before = len(buf_wire["c"])
        base_opened0 = int(((shadow_state().get("stats") or {}).get("score") or {}).get("opened", 0))
        t_ms = buf_wire["t"][-1] + 60_000
        # شمعة حية (x=false): تحديث الحية فقط — بلا تنقيط إطلاقاً
        on_kline({"e": "kline", "s": "WIREUSDT",
                  "k": {"i": "1m", "t": t_ms, "o": "110", "h": "111", "l": "109",
                        "c": "110.5", "v": "900", "x": False}})
        check("V7 تدقيق: الشمعة الحية تُحدّث الحية فقط — بلا تنقيط",
              buf_wire["live"] == 110.5 and len(buf_wire["c"]) == n_before
              and "WIREUSDT" not in SCORE_LAST_T
              and int(((shadow_state().get("stats") or {}).get("score") or {}).get("opened", 0)) == base_opened0)
        # الإغلاق (x=true): تنقيط واحد + شمعة مُلحقة + قياس ظلي
        on_kline({"e": "kline", "s": "WIREUSDT",
                  "k": {"i": "1m", "t": t_ms, "o": "110", "h": "111", "l": "109",
                        "c": "110.8", "v": "1200", "x": True}})
        once_opened = int(((shadow_state().get("stats") or {}).get("score") or {}).get("opened", 0))
        check("V7 تدقيق: الإغلاق يلحق الشمعة ويُطلق التنقيط مرة واحدة (ظلي)",
              len(buf_wire["c"]) == n_before + 1 and SCORE_LAST_T.get("WIREUSDT") == t_ms
              and once_opened == base_opened0 + 1)
        # إعادة إرسال نفس الشمعة (إعادة اتصال): بلا تقييم مكرر
        on_kline({"e": "kline", "s": "WIREUSDT",
                  "k": {"i": "1m", "t": t_ms, "o": "110", "h": "111", "l": "109",
                        "c": "110.8", "v": "1200", "x": True}})
        check("V7 تدقيق: إعادة إرسال الشمعة المغلقة لا تُقيّم مرتين",
              int(((shadow_state().get("stats") or {}).get("score") or {}).get("opened", 0)) == once_opened)
        # رسوم مشوهة/عملة غريبة: لا انهيار
        on_kline(None); on_kline({"e": "trade"}); on_kline({"e": "kline", "s": "X",
                  "k": {"i": "5m", "t": 1, "x": True}})
        check("V7 تدقيق: رسوم kline مشوهة لا تُسقط الحلقة", True)
    finally:
        globals()["SYMBOLS"], globals()["SYMBOLS_SET"] = _orig_syms, _orig_set
        CANDLES.pop(("WIREUSDT", "1m"), None)
        SCORE_LAST_T.pop("WIREUSDT", None); SCORE_ATTEMPT.pop("WIREUSDT", None)
        globals().update(_saved)

    # 33) الأخوات تحترم المطالبة الذرية (من يُدار بالفعل لا يُلمس)
    globals()["force_exit_position"] = _fake_exit
    try:
        STATE["positions"] = {
            "PC": {"symbol": "SB4USDT", "entry": 100.0, "last_price": 99.0, "opened_at": now_ts - 300},
            "PO2": {"symbol": "SB4USDT", "entry": 100.0, "last_price": 100.5, "opened_at": now_ts - 300},
        }
        EXIT_CLAIMED.add("PC")
        check("V7 تدقيق: الأخوات تتجاهل مركزاً قيد الإعدام (مطالبة ذرية)",
              sibling_review_once() == 0)
        EXIT_CLAIMED.discard("PC")
    finally:
        globals()["force_exit_position"] = _orig_exit
    STATE["positions"] = {}

    # 34) تدقيق بنيوي: الأسلاك مثبتة في المصدر الصحيح
    import inspect as _insp
    ps_src = _insp.getsource(globals()["process_signal"])
    gates_src = _insp.getsource(globals()["_entry_gates_open"])
    uni_src = _insp.getsource(globals()["universe_loop"])
    launch_src = _insp.getsource(globals()["launch_entry"])
    check("V7 تدقيق: A/B في مسار الحجز + الفوضى في البوابات + تنظيف تدوير الكون + الزحمة في الإطلاق",
          "mode_cooldown()" in ps_src and "pyramid_allowed(" in ps_src
          and "chaos_blocked()" in gates_src
          and "SCORE_LAST_T.pop" in uni_src and "SCORE_ATTEMPT.pop" in uni_src
          and "crowd_register_and_check()" in launch_src)
    _close_fn = (globals().get("finalize_position_event") or globals().get("finalize_position")
                 or globals().get("finalize"))
    check("V7 تدقيق: مزاج العملة مقيّد في مسار الإغلاق النهائي",
          _close_fn is not None and _insp.getsource(_close_fn).count("flavor_update(") >= 1)

    # 35) إشارة score عبر veto_reason: بلا انهيار مع بيانات ناقصة
    try:
        vr_out = veto_reason("GHOSTUSDT", {"symbol": "GHOSTUSDT", "trigger": "score",
                                           "score": 60.0, "atr": 0.0, "atr_1m": 0.5, "vr": 1.0})
        check("V7 تدقيق: veto_reason يتعامل مع إشارة score لعملة شبحية بلا بيانات",
              vr_out is None or isinstance(vr_out, str))
    except Exception:
        check("V7 تدقيق: veto_reason يتعامل مع إشارة score لعملة شبحية بلا بيانات", False)
...
async def amain(args):
    ...
    async with aiohttp.ClientSession(connector=connector,
        ...
        tasks = [
            ...
            asyncio.create_task(sibling_loop(), name="sibling-loop"),
            asyncio.create_task(insights_loop(), name="insights-loop"),
        ...
        log(
            f"تشغيل NOVA V7.0 UNIFIED AI-HFT [رشاش + قنّاص] | {regime_text()} | Spot فقط | "
