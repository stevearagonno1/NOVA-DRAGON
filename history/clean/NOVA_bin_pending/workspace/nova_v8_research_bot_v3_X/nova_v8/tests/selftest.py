...
def _check_exit_engine():
    ...
    Runs with the default fib trail active to exercise that branch too."""
...
def _check_fib_trail_and_gap():
    """Fibonacci retracement trail must be able to lock a trailing exit, and a
    gap past a stop must fill at the bar OPEN (not at the stale stop level)."""
    from nova_v8.execution import Position, ExitEngine
    from nova_v8 import config as C
    basis = C.TRAIL_BASIS

    # ---- gap handling: long opens far below its hard stop -> fill at open ----
    pos = Position("T", 1, 100.0, 100.0, 0.01, "EMA", "صاعد", notional=20.0,
                   entry_bar=0)
    eng = ExitEngine(); eng.arm(pos)               # hard stop ~98.85
    gapped_open = 97.0                             # big gap below the stop
    res = eng.update(pos, gapped_open, 97.2, 96.8, 1, "صاعد", 97.0)
    assert res is not None, "gap below stop should force an exit"
    assert abs(res[1] - gapped_open) < 1e-6, "gap fill must be at bar open"

    # ---- Fibonacci trail locks a profit when a big run gives back ----
    if basis == "fib":
        pos = Position("T", 1, 100.0, 100.0, 0.01, "EMA", "صاعد",
                       notional=20.0, entry_bar=0)
        eng = ExitEngine(); eng.arm(pos)
        # climb to ~115, then fall back; a 0.618-fib trail should stop it well
        # above entry (protecting part of the run).
        prev = 100.0
        fill = None
        closes = list(np.linspace(100.0, 115.0, 300)) + list(np.linspace(115.0, 101.0, 80))
        for i in range(1, len(closes)):
            c = closes[i]
            o = prev
            bar = (o, max(o, c) * 1.0005, min(o, c) * 0.9995, c)
            res = eng.update(pos, bar[0], bar[1], bar[2], i, "صاعد", c)
            if res:
                fill = res
                break
        ...
        if fill is None:
            fill = ("نهاية", closes[-1])
        assert fill[1] > pos.entry_px, "fib trail exit should lock a profit"
...
def run(tmp_root: Path | None = None) -> int:
    ...
    old_syms = C.SYMBOLS
    old_res, old_rep, old_stab, old_flip = (C.RESULT_PATH, C.REPORT_PATH,
                                         C.STABILITY_PATH, C.FLIP_PATH)
    ...
    C.FLIP_PATH = out / "slippage_flip_report.txt"
    C.REGIME_TF = "15m"
    try:
        print("unit: grid...")
        _check_grid_unit()
        ...
        _check_exit_engine()
        print("unit: fib trail + gap...")
        _check_fib_trail_and_gap()
        print("unit: btc pass...")
        _check_btc_pass()
        ...
        C.SYMBOLS = old_syms
        C.RESULT_PATH, C.REPORT_PATH, C.STABILITY_PATH, C.FLIP_PATH = (old_res,
                                             old_rep, old_stab, old_flip)
        if keep is None:
