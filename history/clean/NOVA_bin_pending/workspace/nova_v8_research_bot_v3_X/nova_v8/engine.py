...
from .execution import Position, ExitEngine, settle_directional, \
    spread_multiplier, net_fraction, GOOD_PROFIT_THRESHOLD
...
def replay_directional(mat: feeds.Matrix) -> list[dict]:
    ...
    def _slip_at(e, regime):
        """Effective slippage (base x vol multiplier) at bar e."""
        if vrA is not None and 0 <= e < n and np.isfinite(float(vrA[e])):
            vr = float(vrA[e])
        else:
            vr = None
        return C.SLIPPAGE_PCT * spread_multiplier(vr, regime)
...
def _write_flip_report(records: list[dict]) -> None:
    ...
    for trig, grp in sorted(by_trig.items(), key=lambda kv: -len(kv[1])):
        row = trig.ljust(12)
        for t in tiers:
            nets = [net_fraction(r["entry_px"], r["exit_px"], r[D["side"]], t)
                    for r in grp]
