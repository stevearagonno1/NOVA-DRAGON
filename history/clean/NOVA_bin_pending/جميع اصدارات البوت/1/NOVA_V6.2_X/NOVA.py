...
def load_state():
    ...
    try:
        ...
        sh = base.get("shadow") if isinstance(base.get("shadow"), dict) else {}
        base["shadow"] = {
            "stats": sh.get("stats") if isinstance(sh.get("stats"), dict) else {},
            "open": sh.get("open") if isinstance(sh.get("open"), dict) else {},
            "history": sh.get("history") if isinstance(sh.get("history"), list) else [],
            "skipped": int(sh.get("skipped", 0) or 0),
        }
...
async def manage_loop():
    ...
    while True:
        ...
        try:
            ...
            if SHADOW_ENABLED:
                _sh_symbols = {p.get("symbol") for p in (STATE.get("shadow", {})
                                                         .get("open") or {}).values()}
                for _ssym in list(_sh_symbols):
                    if _ssym:
