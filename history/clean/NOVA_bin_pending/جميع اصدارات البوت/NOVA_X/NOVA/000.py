...
بوت سكالبينج Spot تلقائي على Binance، مضبوط افتراضياً على Binance Spot Demo Mode.
...
الإعداد:
    ...
    export BINANCE_API_KEY='مفتاح Binance Demo Spot'
    export BINANCE_API_SECRET='سر Binance Demo Spot'
...
لا يوجد ضمان للربح؛ ابدأ بـ Demo Mode واختبر السلوك قبل أي أموال حقيقية.
"""
...
BINANCE_ENV = os.getenv("BINANCE_ENV", "demo").strip().lower()
...
ZONE_MAX_AGE_CANDLES = env_int("ZONE_MAX_AGE_CANDLES", 120)
TOUCH_LOOKBACK_BARS = env_int("TOUCH_LOOKBACK_BARS", 8)
...
HELP_TEXT = (
    ...
    "ثم 4 شموع خضراء + BOS + FVG، وPullback، وMACD سريع وفوليوم متحسن على 1m."
)
...
def run_scan(force=False):
    ...
    if not SYMBOL_RULES:
        ...
        if force:
            send_message("⚠️ لا توجد قواعد Binance محمّلة؛ تحقق من الوصول إلى Binance Demo API.")
        SCAN_LOCK.release()
        return None
