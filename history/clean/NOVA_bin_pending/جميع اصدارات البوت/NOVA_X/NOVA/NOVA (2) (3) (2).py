...
# -*- coding: utf-8 -*-
"""
NOVA HMSM SCALPER v3.1 — Zero-Wait Microstructure Scalper (Stealth)
...
6) الحجم: 20$ ثابتة لكل مركز (تتجاوز الحد الأدنى للطلب في Binance وتمنع
   رفض OCO/Dust بالكامل). Pyramiding: حتى 3 مراكز متزامنة مستقلة لكل عملة،
   لكل مركز معرّف فريد (pid) في الذاكرة وOCO خاص يُدار منفصلاً.
7) الحماية الواسعة ضد "فخ ATR الدقيقة" (ضجيج 1m يوقف الصفقة بلا معنى):
   وقف 3×ATR وهدف 6×ATR (عائد/مخاطرة 1:2 بالضبط)، وTrailing لا يتفعل إلا
   بعد 3.5×ATR أي بعد تجاوز منطقة الخطر كاملة، ثم يتبع بمسافة ATR لحظية.
...
الوضع الصامت التام (Stealth):
• لا تُرسل أي رسالة تلقائية إلى Telegram إطلاقاً — لا دخول ولا إغلاق ولا
  Trailing ولا Dust ولا إنقاذ ولا نبضة. كل ذلك يُسجَّل في bot.log فقط.
• الردود تُرسل فقط عندما يطلب المستخدم بنفسه أمراً (/status، /help، /menu،
  /scan، /coin، /prices، /pause، /resume، /cancelall، /panic، /kill، /unkill).
...
# Trailing/Dust/إنقاذ/نبضة). الردود تُرسل فقط على أوامر المستخدم اليدوية.
STEALTH_MODE = True
...
# مضاعفات واسعة لتفادي "فخ ATR الدقيقة" (ضجيج 1m): وقف 3×ATR وهدف 6×ATR
# (عائد/مخاطرة 1:2 بالضبط)، والتفعيل بعد 3.5×ATR أي بعد تجاوز منطقة الخطر.
ATR_PERIOD = max(2, env_int("ATR_PERIOD", 14))
STOP_ATR_MULT = env_float("STOP_ATR_MULT", 3.0)
TP_ATR_MULT = env_float("TP_ATR_MULT", 6.0)
TRAIL_ACTIVATION_ATR_MULT = env_float("TRAIL_ACTIVATION_ATR_MULT", 3.50)
...
SESSION.headers.update({"User-Agent": "NOVA-HMSM-SCALPER/3.1"})
try:
...
def execute_entry(signal):
    ...
    try:
        ...
        log(f"دخول HMSM {symbol} pid={pid} entry={fmt_price(symbol, entry)} qty={position['qty']} "
            ...
            f"trail@{fmt_price(symbol, activation_price)} protection={protection['type']}")
        return True
    except BinanceError as exc:
...
def activate_trailing(pid, position, current_price):
    ...
    try:
        ...
        save_state()
        log(f"تفعيل Trailing {pid}: سعر={fmt_price(symbol, current_price)} تجاوز {fmt_price(symbol, position.get('activation_price'))} "
            f"ATR={atr_now:.8g} مسافة={TRAIL_DISTANCE_ATR_MULT}×ATR delta={trail_bips}bips")
        return True
    except BinanceError as exc:
...
HELP_TEXT = (
    "🤖 <b>NOVA HMSM SCALPER v3.1 — Stealth</b>\n\n"
    "/status تقرير الأداء\n/scan فحص HMSM فوري\n/coin SOL تحليل عملة\n"
    ...
    "الاستراتيجية: MSS + FVG + Order Flow في اللحظة نفسها → LIMIT BUY فوري بحجم 20$.\n"
    "حتى 3 مراكز متزامنة لكل عملة، كل مركز بمعرّف وOCO مستقل.\n"
    "وقف 3×ATR وهدف 6×ATR (عائد/مخاطرة 1:2) وTrailing يتفعل بعد 3.5×ATR.\n"
    "لا انتظار Pullback، لا مؤشرات بطيئة، ولا قواطع خسائر متتالية.\n"
    "الوضع الصامت: لا رسائل تلقائية إطلاقاً — اطلب /status متى شئت."
)
...
def handle_command(text, chat):
    ...
    if command in ("/start", "/menu"):
        send_message("🎛️ <b>NOVA HMSM SCALPER v3.1</b>\nاختر عملية:", main_keyboard(), chat)
    elif command == "/help":
        send_message(HELP_TEXT, main_keyboard(), chat)
    elif command == "/status":
        send_message(status_text(), main_keyboard(), chat)
...
def selftest():
    print("NOVA HMSM SCALPER v3 SELFTEST")
    results = []
    ...
    check(
        ...
        plan is not None and plan["trail_bips"] == 60
        and abs(plan["activation_price"] - (100.0 + TRAIL_ACTIVATION_ATR_MULT * 0.5)) < 1e-9
        and abs(plan["trailing_distance"] - 0.6) < 1e-9,
    )
    ...
    # مواصفات الحجم والمخاطرة: 20$ ثابتة + عائد/مخاطرة 1:2 + مضاعفات واسعة
    check(
        "حجم 20$ + 1:2 R:R + مضاعفات ATR الواسعة",
        TRADE_NOTIONAL_D == Decimal("20")
        and abs(TP_ATR_MULT / STOP_ATR_MULT - 2.0) < 1e-9
        and abs(STOP_ATR_MULT - 3.0) < 1e-9
        and abs(TP_ATR_MULT - 6.0) < 1e-9
        and abs(TRAIL_ACTIVATION_ATR_MULT - 3.5) < 1e-9,
    )

    # الوضع الصامت: الرسائل التلقائية محجوبة والردود على الأوامر فقط
    global tg_call, TELEGRAM_TOKEN, TELEGRAM_CHAT_ID, DRY_RUN
...
def main():
    ...
    log(
        f"تشغيل NOVA HMSM SCALPER v3.1 [صامت] | {regime_text()} | Spot فقط | Zero-Wait | "
        f"أفضل {TOP_N_SYMBOLS} زوجاً | حجم {TRADE_NOTIONAL_D}$ | حتى {MAX_POSITIONS_PER_SYMBOL} مراكز/عملة | "
        f"وقف {STOP_ATR_MULT}×ATR هدف {TP_ATR_MULT}×ATR تتبع بعد {TRAIL_ACTIVATION_ATR_MULT}×ATR | "
        ...
        try:
            ...
            if not startup_ready and now - last_startup_retry >= 30:
                ...
                try:
                    startup_checks()
                    startup_ready = True
                    log("عاد اتصال Binance؛ الرادار جاهز بأقصى سرعة")
                except BinanceError as exc:
