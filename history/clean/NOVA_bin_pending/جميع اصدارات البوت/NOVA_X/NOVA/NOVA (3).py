...
# -*- coding: utf-8 -*-
"""
NOVA HMSM SCALPER v4.0 — Mobile 5M Stealth Scalper (Termux Edition)
بوت سكالبينج Spot على Binance مبني على نموذج Hybrid Microstructure
Scalping Model (HMSM) — مُعاد تهيئته للجوال (Termux) وجودة أعلى.

الفلسفة المعمارية:
1) Zero-Wait: لا حفظ لإعدادات ولا انتظار Pullback إطلاقاً. كل دورة فحص تقرأ
   الشموع مباشرة؛ إذا تحققت الأعمدة في اللحظة نفسها يُنفَّذ LIMIT BUY
   فوري على سعر Ask، وإلا تُتجاوز العملة فوراً إلى التالية.
2) محرك التنفيذ على فريم 5m (جودة لا كمية): جميع حسابات MSS وFVG وATR
   تُقرأ الآن من الشموع خامسة الدقائق حصراً — إشارات أنظف وضجيج أقل
   وفرص أهدأ تمنح الصفقة وقتاً حقيقياً للوصول إلى هدفها.
3) الفلتر الكلي (Macro Trend Filter): قبل أي دخول يُجلب فريم 15m ويُحسب
   عليه EMA200؛ يجب أن يكون إغلاق 5m الحالي أعلى منه بشكل صارم، وإلا
   تُتخطى العملة فوراً بلا استثناء (لا شراء ضد الترند الأكبر أبداً).
4) الأعمدة الثلاثة (يجب أن تجتمع في اللحظة نفسها على 5m):
   • MSS  : كسر هيكل صاعد (Market Structure Shift) بإزاحة قوية على 5m.
   • FVG  : فجوة قيمة عادلة (Fair Value Gap) صاعدة حديثة وغير معبأة على 5m.
   • Flow : اختلال تدفق الأوامر من دفتر Level-1 عبر book_stats لصالح الطلب.
5) لا مؤشرات بطيئة على فريم الإشارة: لا MACD ولا RSI ولا Fibonacci.
   السعر + الدفتر فقط، ويُضاف EMA200 على فريم 15m حصراً كبوابة اتجاه كلي.
6) الكون الديناميكي الموفّر للبطارية: أفضل 30 زوجاً فقط حسب حجم التداول
   24 ساعة، تحديث دوري كل 10 دقائق، بلا عملة قائدة ولا قوائم ثابتة.
7) تهدئة الجوال (Termux): فحص كل 15 ثانية، 5 خيوط فحص فقط، وتزامن HTTP
   بحد 8 طلبات متزامنة — لا حرارة زائدة للمعالج ولا اصطدام بحدود معدل
   طلبات Binance (429/حظر API).
8) الحجم: 20$ ثابتة لكل مركز (تتجاوز الحد الأدنى للطلب في Binance وتمنع
   رفض OCO/Dust بالكامل). Pyramiding: حتى 3 مراكز متزامنة مستقلة لكل
   عملة، لكل مركز معرّف فريد (pid) في الذاكرة وOCO خاص يُدار منفصلاً،
   مع تهدئة صارمة 300 ثانية (SAME_SYMBOL_COOLDOWN) بين فتح مركزين على
   العملة نفسها — لا تُفتح الأهرامات الثلاثة في الشمعة الواحدة أبداً.
9) Set & Forget صارم: OCO واحد ثابت فور الدخول — وقف 2×ATR(5m) وهدف
   4×ATR(5m) (عائد/مخاطرة 1:2 رياضي خالص) — لا يُلمس الأمر بعد وضعه
   أبداً؛ Binance وحده ينفذ الوقف أو الهدف. لا Trailing Stop في أي صورة
   إطلاقاً ولا أي منطق Trailing في الكود.
10) لا قواطع خسائر متتالية ولا حدود خسارة يومية: البوت يتداول إلى ما لا
    نهاية، مع الاحتفاظ بعدّاد الخسائر المتتالية للتقرير فقط.

الوضع الصامت التام (Stealth) + إشعار بدء واحد:
• عند الإقلاع بنجاح والتحقق من اتصال Binance يُرسل البوت تقرير status_text()
  مرة واحدة بالضبط إلى Telegram (notify_startup) ثم يلتزم الصمت المطلق.
• لا تُرسل أي رسالة تلقائية بعد ذلك إطلاقاً — لا دخول ولا إغلاق ولا Dust
  ولا إنقاذ ولا نبضة. كل ذلك يُسجَّل في bot.log فقط.
• الردود تُرسل فقط عندما يطلب المستخدم بنفسه أمراً (/start، /status، /reset،
  /help، /menu، /scan، /coin، /prices، /pause، /resume، /cancelall، /panic،
  /kill، /unkill).
• ضمان معماري: send_message يرفض أي استدعاء بلا chat_id صريح.
...
التشغيل:
    python NOVA_4.py --selftest
    python NOVA_4.py --dry-run
    python NOVA_4.py
    python NOVA_4.py --once
...
# الصمت التام: لا تُرسل أي رسالة تلقائية إلى Telegram إطلاقاً (دخول/إغلاق/
# Dust/إنقاذ/نبضة). الردود تُرسل فقط على أوامر المستخدم اليدوية.
STEALTH_MODE = True

# ── الكون الديناميكي: أفضل 30 زوجاً فقط حسب حجم 24 ساعة (توفير للبطارية) ────
TOP_N_SYMBOLS = 30
...
# ── محرك HMSM على فريم 5m + الفلتر الكلي EMA200 على فريم 15m ────────────────
SIGNAL_INTERVAL = "5m"
# الفلتر الكلي الصارم: إغلاق 5m الحالي أعلى بشكل صارم من EMA200(15m)
# (لا شراء في الترند الأكبر الهابط إطلاقاً — تحت الخط → تخطٍ فوري للعملة)
TREND_INTERVAL = "15m"
...
# فريم 15m يحتاج EMA200 + تسخين كافٍ: يُجلب له وحده بحد أكبر
KLINE_LIMITS = {SIGNAL_INTERVAL: KLINE_LIMIT, TREND_INTERVAL: clamp(EMA_TREND_PERIOD + 50, 210, 1000)}
MIN_CLOSED_CANDLES = max(30, env_int("MIN_CLOSED_CANDLES", 60))
# TTL أطول (بيانات 5m/15m لا تتقادم بسرعة): طلبات أقل = بطارية أبرد
KLINE_TTL = {"5m": max(5, env_int("KLINE_TTL_5M", 30)), "15m": max(10, env_int("KLINE_TTL_15M", 240))}
...
# ── ATR والمخارج الثابتة (Set & Forget — لا Trailing إطلاقاً) ────────────────
# وقف 2×ATR وهدف 4×ATR: عائد/مخاطرة 1:2 رياضي خالص. OCO واحد ثابت يُوضع فور
# الدخول ولا يُلمس بعدها أبداً — Binance وحده ينفذ الوقف أو الهدف.
ATR_PERIOD = max(2, env_int("ATR_PERIOD", 14))
STOP_ATR_MULT = env_float("STOP_ATR_MULT", 2.0)
TP_ATR_MULT = env_float("TP_ATR_MULT", 4.0)
...
# ── التنفيذ: 20$ ثابتة + حتى 3 مراكز متزامنة لكل عملة ───────────────────────
TRADE_NOTIONAL_D = Decimal("20")        # ثابت بالمواصفات: عشرون دولاراً لكل مركز
                                        # (يتجاوز الحد الأدنى للطلب في Binance ويمنع رفض OCO/Dust)
MAX_POSITIONS_PER_SYMBOL = 3            # ثابت بالمواصفات: Pyramiding مستقل
# تهدئة الأهرامات: لا مركز جديد على العملة نفسها قبل مضي 300 ثانية (5 دقائق)
# من فتح آخر مركز — يمنع فتح الأهرامات الثلاثة في الشمعة الواحدة نفسها.
...
# ── تحسين الجوال (Termux): إيقاع هادئ يوفر البطارية ويمنع سخونة المعالج ──────
SCAN_EVERY = max(5, env_int("SCAN_EVERY", 15))
MANAGE_EVERY = max(2, env_int("MANAGE_EVERY", 10))
# فحص أفضل 30 زوجاً بـ 5 خيوط فقط وتزامن HTTP بحد 8: لا حرارة ولا 429
SCAN_WORKERS = max(1, min(16, env_int("SCAN_WORKERS", 5)))
HTTP_CONCURRENCY = max(2, min(32, env_int("HTTP_CONCURRENCY", 8)))
...
SESSION.headers.update({"User-Agent": "NOVA-HMSM-SCALPER/4.0"})
try:
...
TICKER_CACHE = {}
KLINE_CACHE = {}
MAX_KLINE_CACHE_ENTRIES = max(64, TOP_N_SYMBOLS * 4)  # 5m + 15m لكل عملة
LAST_UNIVERSE_REFRESH = 0.0
LAST_USED_WEIGHT = 0
RATE_BACKOFF_UNTIL = 0.0
# إشعار البدء: يُرسل مرة واحدة بالضبط بعد نجاح التحقق من اتصال Binance
...
def migrate_positions(raw):
    ...
    for key, position in (raw or {}).items():
        ...
        position["pid"] = pid
        # تنظيف حقول Trailing من ذاكرة الإصدارات القديمة (Set & Forget)
...
def ema(values, period):
    """EMA قياسي — يستخدم حصراً لبوابة الاتجاه الكلي EMA200 على فريم 5m."""
    result = [None] * len(values)
    if period <= 0 or len(values) < period:
        return result
...
def refresh_universe(force=False):
    """أفضل 30 زوجاً ديناميكياً حسب حجم التداول 24 ساعة — بلا قوائم ثابتة."""
...
# ══════════════════════════════════════════════════════════════════════════════
# 5) تحليل فريم الإشارة 5m (سريع وخفيف) + دفتر Level-1
# ══════════════════════════════════════════════════════════════════════════════


def analyze_frame(raw, interval):
    """تحليل خفيف: شموع مغلقة + ATR(فريم الإشارة 5m) + نسبة حجم. لا مؤشرات بطيئة."""
    if not raw:
        return None
    ...
    if not force:
        with KLINE_CACHE_LOCK:
            ...
            if cached and now - cached["time"] < KLINE_TTL.get(interval, 5):
                return cached["frame"]
    raw = CLIENT.klines(symbol, interval, limit=KLINE_LIMITS.get(interval, KLINE_LIMIT))
...
def macro_trend_ok(trend_frame, price):
    """الفلتر الكلي (Macro Trend Filter — قاعدة Spot): يجب أن يكون إغلاق 5m
    الحالي أعلى بشكل صارم من EMA200 المحسوبة على فريم 15m — لا شراء في
    الترند الأكبر الهابط إطلاقاً."""
    if not trend_frame or len(trend_frame.get("c", [])) < EMA_TREND_PERIOD:
        return False  # لا يقين اتجاهي (بيانات أقل من EMA200) → لا صفقة
    series = ema(trend_frame["c"], EMA_TREND_PERIOD)
    value = series[-1] if series else None
    return value is not None and float(price) > value


def scan_one_symbol(symbol):
    """عامل فحص مستقل: الفلتر الكلي EMA200(15m) أولاً ثم MSS + FVG + Flow على 5m
    الآن → إشارة، وإلا None فوراً."""
    rule = SYMBOL_RULES.get(symbol)
    if not rule or rule.get("status") != "TRADING":
        return None
    if D(rule.get("min_notional", "0")) > TRADE_NOTIONAL_D * Decimal("0.85"):
        return None  # مركز 20$ لا يمكن حمايته تحت الحد الأدنى للطلب
    try:
        # الفلتر الكلي قبل أي تحليل 5m: فريم 15m وحساب EMA200
        trend_frame = fetch_frame_cached(symbol, TREND_INTERVAL)
        frame = fetch_frame_cached(symbol, SIGNAL_INTERVAL)
        if not frame:
            return None
        if not macro_trend_ok(trend_frame, frame["live"]):
            return None  # إغلاق 5m تحت EMA200(15m): ترند أكبر هابط → تخطٍ فوري
        ...
        return {
            ...
            "stop": price - STOP_ATR_MULT * atr_value,
            "target": price + TP_ATR_MULT * atr_value,
...
# ══════════════════════════════════════════════════════════════════════════════
# 7) التنفيذ: LIMIT فوري + OCO ثابت مستقل لكل مركز (Set & Forget)
# ══════════════════════════════════════════════════════════════════════════════
...
def wait_limit_fill(symbol, order_id):
    ...
    while True:
        ...
        # PENDING_CANCEL حالة عابرة: قد تتعبأ الكمية المتبقية قبل اكتمال الإلغاء؛
        # لا يجوز اعتبارها يقيناً — نواصل الاستقصاء حتى الحالة النهائية.
...
def reserve_entry(symbol):
    """حجز ذري: حتى 3 مراكز متزامنة لكل عملة (Pyramiding مستقل) + تهدئة
    SAME_SYMBOL_COOLDOWN بين مراكز العملة نفسها (لا أهرامات في شمعة واحدة)."""
...
def execute_entry(signal):
    ...
    if not live_execution_allowed():
        if DRY_RUN:
            log(f"إشارة مؤكدة بدون تنفيذ (DRY-RUN): {symbol} @ {fmt_price(symbol, signal['price'])} | ATR {signal['atr']:.8g}")
        else:
    ...
    try:
        ...
        save_state()
        log(f"دخول HMSM {symbol} pid={pid} entry={fmt_price(symbol, entry)} qty={position['qty']} "
            ...
            try:
                ...
                if attempts > 60:
                    with STATE_LOCK:
                        STATE["pending_entries"].pop(client_id, None)
                    save_state()
                    log(f"مصالحة {symbol}: تعذر حسم أمر معلق بعد {attempts} محاولة؛ حُذف السجل — تحقق من محفظتك")
                else:
...
def recover_filled_entry(client_id, record, order):
    ...
    if not rule:
        ...
        save_state()
        return
    # التصفية/الإيقاف اليدوي مقدَّم على فتح مراكز جديدة: لا "إحياء" بعد /panic أو /kill
    if STATE.get("paused") or kill_switch_active():
        exit_order = emergency_sell(symbol, executed, "استعادة تعبئة أثناء الإيقاف/التصفية")
        if exit_order:
            account_salvage(symbol, quote_filled, exit_order, "بيع إنقاذ: تعبئة أثناء الإيقاف/التصفية")
        else:
            add_dust(symbol, executed, quote_filled, "تعذر بيع تعبئة أثناء الإيقاف")
        with STATE_LOCK:
            drop()
        save_state()
        return
    ...
    try:
        # ATR الحماية تُقرأ من فريم الإشارة 5m حصراً
        frame = fetch_frame_cached(symbol, SIGNAL_INTERVAL, force=True)
    ...
    log(f"مصالحة: استعادة مركز {pid} على {symbol} entry={fmt_price(symbol, entry)} "
        f"stop={fmt_price(symbol, stop_price)} target={fmt_price(symbol, target_price)}")
...
def monitor_position(pid, position, open_ids):
    symbol = position["symbol"]
    order_ids = [oid for oid in position.get("order_ids", []) if oid is not None]
    if not order_ids:
        # لا أرجل مسجلة: استعدها من قائمة OCO إن وُجد معرفها
        list_id = position.get("order_list_id")
        if list_id:
            try:
                detail = CLIENT.order_list(order_list_id=list_id)
                legs = [o.get("orderId") for o in detail.get("orders", []) if o.get("orderId") is not None]
            except BinanceError as exc:
                log(f"تعذر جلب قائمة OCO لـ {pid}: {exc}")
                legs = []
            if legs:
                with STATE_LOCK:
                    position["order_ids"] = legs
                save_state()
                log(f"مصالحة حماية {pid}: استُعيدت الأرجل {legs}")
                order_ids = legs
            else:
                return  # OCO قائم لكن تعذر جلب أرجله؛ محاولة في دورة قادمة
        else:
            # لا حماية قابلة للتتبع إطلاقاً: خروج طارئ لحماية رأس المال
            log(f"مركز {pid} بلا حماية مسجلة؛ خروج طارئ")
            exit_order = emergency_sell(symbol, D(position.get("qty", "0")), "مركز بلا حماية مسجلة")
            if exit_order:
                finalize_position(pid, position, exit_order, "خروج طارئ: مركز بلا حماية")
            else:
                failures = int(position.get("sell_failures", 0)) + 1
                with STATE_LOCK:
                    position["sell_failures"] = failures
                if failures >= 5:
                    add_dust(symbol, D(position.get("qty", "0")), Decimal("0"), "مركز بلا حماية وفشل البيع مراراً")
    ...
    if open_ids is not None:
        ...
        if filled:
            finalize_position(pid, position, filled, "تنفيذ OCO/STOP على Binance")
            return
    # Set & Forget: OCO ثابت لا يُلمس أبداً بعد وضعه — Binance وحده ينفذ الوقف أو الهدف
...
def manage_positions():
    ...
    try:
        ...
        for pid, position in positions:
            try:
                monitor_position(pid, position, open_ids)
            except Exception as exc:
...
def panic_close_all():
    cancel_all_orders()
    # أمر الدخول المعلق جزء من التصفية أيضاً: ألغِه لمنع "إحياء" صفقة بعد الـ panic
    for record in list(STATE.get("pending_entries", {}).values()):
        symbol = record.get("symbol", "")
        order_id = record.get("order_id")
        if not order_id:
            continue
...
def status_text():
    """تقرير الحالة — الصيغة العربية المطلوبة حرفياً (بدون قسم العملات النشطة)."""
    m = STATE.get("metrics", {})
...
HELP_TEXT = (
    "🤖 <b>NOVA HMSM SCALPER v4.0 — Mobile 5M (Stealth + Set &amp; Forget)</b>\n\n"
    ...
    "الاستراتيجية: MSS + FVG + Order Flow على فريم 5m → LIMIT BUY فوري بحجم 20$.\n"
    "الفلتر الكلي: إغلاق 5m الحالي أعلى بشكل صارم من EMA200 على فريم 15m وإلا تُتخطى العملة.\n"
    "أفضل 30 زوجاً فقط | فحص كل 15 ثانية بـ 5 خيوط (وضع الجوال — بطارية أبرد).\n"
    "حتى 3 مراكز متزامنة لكل عملة بتهدئة صارمة 300ث بينها، كل مركز بمعرّف وOCO مستقل.\n"
    "وقف 2×ATR(5m) وهدف 4×ATR(5m) (عائد/مخاطرة 1:2) — OCO ثابت لا يُلمس بعد وضعه.\n"
    "لا Trailing، لا انتظار Pullback، ولا قواطع خسائر متتالية.\n"
    "الوضع الصامت: رسالة بدء واحدة عند الإقلاع ثم صمت تام — اطلب /status متى شئت."
)
...
def send_message(text, keyboard=None, chat_id=None):
    ...
    أي استدعاء تلقائي بلا chat_id يُرفض ويُسجل في السجل — لا رسائل خارجة إطلاقاً."""
...
def notify_startup():
    """إشعار البدء الوحيد: يرسل تقرير status_text() مرة واحدة بالضبط عند نجاح
    الإقلاع والتحقق من اتصال Binance، ثم يلتزم البوت الصمت التام بعدها."""
    global STARTUP_NOTIFIED
    if STARTUP_NOTIFIED:
        return False
    STARTUP_NOTIFIED = True  # يُضبط قبل الإرسال: مرة واحدة بالضبط لا أكثر
    if not TELEGRAM_TOKEN or not TELEGRAM_CHAT_ID:
        log("إشعار البدء (Telegram غير مفعّل): " + status_text().replace("\n", " | "))
        return False
    sent = send_message(status_text(), chat_id=TELEGRAM_CHAT_ID)
...
def coin_report(symbol):
    try:
        ...
        fvg = detect_fvg(frame)
        book = book_stats(symbol)
        trend_ok = macro_trend_ok(fetch_frame_cached(symbol, TREND_INTERVAL), frame["live"])
        if mss:
        ...
        lines = [
            f"⚡ <b>{coin_name(symbol)} — HMSM 5m</b>",
            f"السعر: <code>{fmt_price(symbol, book['ask'])}</code> | سبريد {book['spread_pct']:.3f}%",
            f"ATR(5m): <b>{frame['atr']:.8g}</b> ({frame['atr_pct']:.3f}%) | حجم الشمعة {frame['volume_ratio']:.2f}x",
            "MSS: " + mss_text,
            "FVG: " + fvg_text,
            "Order Flow: " + flow_text,
            "الاتجاه: " + ("✅ إغلاق 5m فوق EMA200(15m)" if trend_ok else "⛔ إغلاق 5m تحت EMA200(15m) — لا شراء"),
            "الإشارة الآن: " + ("🟢 MSS + FVG + Flow + اتجاه صاعد" if (mss and fvg and trend_ok) else "غير متحققة"),
...
def handle_command(text, chat):
    ...
    if command in ("/start", "/menu"):
        send_message("🎛️ <b>NOVA HMSM SCALPER v4.0</b>\nاختر عملية:", main_keyboard(), chat)
    elif command == "/help":
        send_message(HELP_TEXT, main_keyboard(), chat)
    elif command == "/status":
        send_message(status_text(), main_keyboard(), chat)
        ...
        save_state()
        send_message("✅ تم تصفير جميع الإحصائيات بنجاح.", main_keyboard(), chat)
    elif command == "/scan":
        send_message("⚡ بدأ فحص HMSM الفوري.", main_keyboard(), chat)
        threading.Thread(target=run_scan, args=(True,), daemon=True).start()
    elif command == "/pause":
...
def run_scan(force=False):
    ...
    try:
        ...
        last_entries = STATE.get("last_entry_at", {})
        position_counts = {}
        ...
        for symbol in SYMBOLS:
            ...
            if now - cooldowns.get(symbol, 0.0) < SYMBOL_COOLDOWN:
                continue
            if now - last_entries.get(symbol, 0.0) < SAME_SYMBOL_COOLDOWN:
                continue  # تهدئة الأهرامات: مركز حديث على هذه العملة
...
def _selftest_frame():
    """شموع تركيبية على فريم الإشارة 5m: شمعة إزاحة تكسر القمة وتصنع FVG غير معبأة."""
    n = 100
    o, h, l, c, v, t = [], [], [], [], [], []
    now_ms = int(time.time() * 1000)
    step_ms = INTERVAL_SECONDS[SIGNAL_INTERVAL] * 1000
    for i in range(n):
        t.append(now_ms - (n - i) * step_ms)
        v.append(1000.0 + (i % 7))
        if i <= 96:
...
def selftest():
    print("NOVA HMSM SCALPER v4.0 SELFTEST")
    results = []
    ...
    # فريم تركيبي + جودة
    raw = _selftest_frame()
    frame = analyze_frame(raw, SIGNAL_INTERVAL)
    check(f"تحليل فريم {SIGNAL_INTERVAL}", frame is not None and frame["atr"] > 0)
    ...
    step_ms = INTERVAL_SECONDS[SIGNAL_INTERVAL] * 1000
    flat_raw = {
        "t": [int(time.time() * 1000) - (n - i) * step_ms for i in range(n)],
        "o": [100.0] * n, "h": [100.5] * n, "l": [99.5] * n, "c": [100.0] * n,
        "v": [1000.0] * n, "live": 100.0,
    }
    flat_frame = analyze_frame(flat_raw, SIGNAL_INTERVAL)
    ...
    rejected_frame = analyze_frame(rejected_raw, SIGNAL_INTERVAL)
    ...
    # Set & Forget: لا آلية Trailing إطلاقاً في الكود
    check(
        "Set & Forget: لا Trailing إطلاقاً",
    ...
    # EMA (لبوابة الاتجاه الكلي على 5m)
    series = ema(list(range(1, 301)), 200)
    check("EMA", series[198] is None and series[199] is not None and 100.0 < series[-1] < 300.0)

    # الفلتر الكلي: إغلاق فريم الإشارة أعلى بشكل صارم من EMA200(15m) أو لا شراء إطلاقاً
    KLINE_CACHE[("SELFTESTUSDT", TREND_INTERVAL)] = {"time": time.time(), "frame": {"c": [100.0] * 250}}
    trend_frame = fetch_frame_cached("SELFTESTUSDT", TREND_INTERVAL)
    check(
        f"الفلتر الكلي EMA200({TREND_INTERVAL})",
        macro_trend_ok(trend_frame, 100.01) is True
        and macro_trend_ok(trend_frame, 100.0) is False
        and macro_trend_ok(trend_frame, 99.99) is False
        and macro_trend_ok(None, 200.0) is False
        and macro_trend_ok({"c": [100.0] * 150}, 200.0) is False,
    )
    KLINE_CACHE.pop(("SELFTESTUSDT", TREND_INTERVAL), None)
    ...
    # تهدئة الأهرامات: لا مركز جديد على العملة نفسها قبل SAME_SYMBOL_COOLDOWN
    STATE["last_entry_at"] = {}
    cool_a = reserve_entry("SELFTESTUSDT")
    release_entry("SELFTESTUSDT")
    STATE["last_entry_at"]["SELFTESTUSDT"] = time.time()
    cool_b = not reserve_entry("SELFTESTUSDT")
    STATE["last_entry_at"]["SELFTESTUSDT"] = time.time() - (SAME_SYMBOL_COOLDOWN + 1.0)
    cool_c = reserve_entry("SELFTESTUSDT")
    release_entry("SELFTESTUSDT")
    STATE["last_entry_at"] = {}
    check(f"تهدئة الأهرامات {SAME_SYMBOL_COOLDOWN}ث بين مراكز العملة نفسها", cool_a and cool_b and cool_c)

    # صيغة تقرير الحالة المطلوبة حرفياً (بدون قسم العملات النشطة نهائياً)
    ...
    lines = text.split("\n")
    check(
        "صيغة /status العربية (بدون العملات النشطة)",
        lines == [
            "إجمالي الصفقات : 5", "",
            "🟢الناجحة : 2", "",
            "🔴الخاسرة : 3", "",
        ...
        and "العملات النشطة" not in text
        and "BTC" not in text and "USDT" not in text,
    )
    STATE["positions"] = {}

    # مواصفات الحجم والمخاطرة: 20$ ثابتة + عائد/مخاطرة 1:2 رياضي خالص
    check(
        "حجم 20$ + 1:2 R:R + مخارج 2×/4× ATR",
        TRADE_NOTIONAL_D == Decimal("20")
        and abs(TP_ATR_MULT / STOP_ATR_MULT - 2.0) < 1e-9
        and abs(STOP_ATR_MULT - 2.0) < 1e-9
        and abs(TP_ATR_MULT - 4.0) < 1e-9,
    )

    # الوضع الصامت: الرسائل التلقائية محجوبة والردود على الأوامر فقط
    global tg_call, TELEGRAM_TOKEN, TELEGRAM_CHAT_ID, DRY_RUN, save_state, STARTUP_NOTIFIED
    saved_token, saved_chat, saved_dry = TELEGRAM_TOKEN, TELEGRAM_CHAT_ID, DRY_RUN
    TELEGRAM_TOKEN, TELEGRAM_CHAT_ID = "fake-token", "42"
    sent_payloads = []
    original_tg_call = tg_call

    def _spy(method, payload=None, retries=3):
    ...
    tg_call = _spy
    DRY_RUN = False
    auto_blocked = send_message("رسالة تلقائية — يجب أن تُحجب") is False and not sent_payloads
    manual_sent = send_message("رد على أمر يدوي", main_keyboard(), chat_id="42") is True and len(sent_payloads) == 1
    # أمر /reset: تصفير كامل للإحصائيات + الرد الحرفي المطلوب حصراً
    STATE["metrics"] = {"trades": 9, "realized_pnl": -3.5, "wins": 4, "losses": 5, "consecutive_losses": 2}
    _real_save_state = save_state
    save_state = lambda: None
    handle_command("/reset", "42")
    save_state = _real_save_state
    reset_ok = (
        STATE["metrics"] == {"trades": 0, "realized_pnl": 0.0, "wins": 0, "losses": 0, "consecutive_losses": 0}
        and len(sent_payloads) == 2
        and sent_payloads[-1]["text"] == "✅ تم تصفير جميع الإحصائيات بنجاح."
    )
    # إشعار البدء: status_text() مرة واحدة بالضبط بعد نجاح الإقلاع ثم صمت تام
    # (يُختبر والتجسس على tg_call ما يزال مفعّلاً)
    STARTUP_NOTIFIED = False
    notify_first = notify_startup()
    notify_again = notify_startup()
    startup_once_ok = (
        notify_first is True
        and notify_again is False
        and len(sent_payloads) == 3
        and sent_payloads[-1]["text"] == status_text()
    )
    STARTUP_NOTIFIED = False
    tg_call = original_tg_call
    ...
    check("إشعار البدء: مرة واحدة بالضبط بعد نجاح الإقلاع", startup_once_ok)
...
def main():
    ...
    try:
        startup_checks()
        startup_ready = True
        notify_startup()  # رسالة البدء الوحيدة بعد التحقق من اتصال Binance
    except BinanceError as exc:
    ...
    log(
        f"تشغيل NOVA HMSM SCALPER v4.0 [جوال/صامت] | {regime_text()} | Spot فقط | فريم الإشارة {SIGNAL_INTERVAL} | "
        f"فلتر كلي EMA200({TREND_INTERVAL}) | فحص {TOP_N_SYMBOLS} زوجاً كل {SCAN_EVERY}ث ({SCAN_WORKERS} خيوط، HTTP×{HTTP_CONCURRENCY}) | "
        ...
        try:
            ...
            if not startup_ready and now - last_startup_retry >= 30:
                ...
                try:
                    startup_checks()
                    startup_ready = True
                    log("عاد اتصال Binance؛ الرادار جاهز")
                    notify_startup()  # إشعار البدء عند أول اتصال ناجح فقط
                except BinanceError as exc:
            ...
            if now - STATE.get("last_heartbeat", 0.0) >= 12 * 3600:
                ...
                save_state()
                log("نبضة 12س: " + status_text().replace("\n", " | "))
            time.sleep(0.5)
