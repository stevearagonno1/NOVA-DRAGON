...
# -*- coding: utf-8 -*-
"""
NOVA HMSM SCALPER v3.0 — Zero-Wait Microstructure Scalper
...
SESSION.headers.update({"User-Agent": "NOVA-HMSM-SCALPER/3.0"})
try:
...
def finalize_position(pid, position, exit_order, reason, exit_price=None):
    ...
    log(f"إغلاق {pid}: pnl={pnl:+.6f}$ reason={reason}")
...
HELP_TEXT = (
    ...
    "الاستراتيجية: MSS + FVG + Order Flow في اللحظة نفسها → LIMIT BUY فوري بحجم 10$.\n"
    "حتى 3 مراكز متزامنة لكل عملة، كل مركز بمعرّف وOCO مستقل.\n"
    "وقف وهدف وTrailing ديناميكية بـ ATR(1m) لكل عملة — لا نسب ثابتة.\n"
...
def handle_command(text, chat):
    ...
    if command in ("/start", "/menu"):
        send_message("🎛️ <b>NOVA HMSM SCALPER v3</b>\nاختر عملية:", main_keyboard(), chat)
    elif command == "/help":
        send_message(HELP_TEXT, main_keyboard(), chat)
    elif command == "/status":
        send_message(status_text(), main_keyboard(), chat)
