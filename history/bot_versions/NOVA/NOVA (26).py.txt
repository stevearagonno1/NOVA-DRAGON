...
1) Macro Trend: يجب أن يكون BTC وETH وBNB، إضافة إلى العملة نفسها، فوق EMA200
   على 15m و30m. إذا فشل أي فلتر، لا توجد دخولات جديدة.
...
LEADERS = ["BTCUSDT", "ETHUSDT", "BNBUSDT"]
DEFAULT_SCALP_SYMBOLS = "BTCUSDT,ETHUSDT,BNBUSDT,SOLUSDT,LINKUSDT,RENDERUSDT,XRPUSDT,DOGEUSDT"
SYMBOLS = []
...
MIN_SCORE = env_float("MIN_SCORE", 78.0)
...
def status_text():
    ...
    lines = [
        ...
        f"اليوم: {m['day']} | صفقات {m['trades']}/{MAX_DAILY_TRADES} | PnL {m['realized_pnl']:+.5f}$",
        f"خسائر متتالية: {m['consecutive_losses']}/{MAX_CONSECUTIVE_LOSSES}",
        f"إعدادات انتظار Pullback: {len(STATE.get('setups', {}))} | مراكز: {len(STATE.get('positions', {}))}/{MAX_POSITIONS}",
    ]
...
HELP_TEXT = (
    ...
    "الدخول لا يحدث إلا بعد: BTC/ETH/BNB والعملة فوق EMA200 على 15m و30m، "
...
def process_candidate(candidate):
    symbol = candidate["symbol"]
    if symbol in STATE.get("positions", {}) or not setup_allowed(symbol, candidate["zone"]):
        return False
    news_text_value = ""
    if AI_GATE_ENABLED:
...
def run_scan(force=False):
    ...
    try:
        ...
        if any(value is None for value in leader_frames.values()):
            log("فشل تحميل فلاتر BTC/ETH/BNB")
            return None
        ...
        for symbol in SYMBOLS:
            ...
            try:
                ...
                if not candidate.get("confirmed"):
                    continue
                # جلب السعر/دفتر الأوامر تم داخل build_candidate عند وجود Pullback.
                prepared = setup_as_candidate(candidate, setup)
                if prepared:
                    best.append(prepared)
                time.sleep(0.10)
            except BinanceError as exc:
        ...
        if best and not STATE.get("paused"):
            process_candidate(best[0])
        save_state()
        log(f"انتهى الفحص؛ مرشحو Pullback المؤكد: {len(best)}")
        return best
    except BinanceError as exc:
        log(f"فشل الفحص: {exc}")
        return None
...
def main():
    ...
    try:
        ...
        log(f"تعذر الاتصال بـ Binance: {exc}")
        if not DRY_RUN:
            raise
    if not live_execution_allowed():
