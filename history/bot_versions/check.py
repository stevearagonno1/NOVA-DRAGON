"""حاضنة تشغيل غير متصلة (V7.0): تُشغّل amain/--once وHub التوجيه مع Binance REST وهمي."""
import asyncio
...
# فحوص V7.0 بعد إقلاع كامل بحاضنة وهمية
ok("V7 محرّكات النسخة الموحّدة محمّلة (تنقيط/أخوات/تسليم/فوضى/زحمة/تقارير)",
   all(callable(getattr(m, n, None)) for n in (
       "scoring_evaluate", "scoring_on_candle_close", "sibling_review_once", "sibling_loop",
       "pyramid_allowed", "chaos_blocked", "crowd_register_and_check",
       "insights_loop", "hourly_report_text", "daily_report_text",
       "bucketize_universe", "normalize_flavor", "flavor_update", "flavor_bad")))
ok("V7 الكون بالدلاء مفعّل (60% سيولة + 25% متحركون)",
   m.UNIVERSE_MODE == "buckets" and m.UNIVERSE_VOL_SHARE == 0.60 and m.UNIVERSE_MOVE_SHARE == 0.25)
ok("V7 الوضع المزدوج A/B (تيربو 5ث) + قاطع الفوضى + تنقيط ≥55 نقطة",
   m.AB_MODE and m.TURBO_COOLDOWN_SEC == 5 and m.CHAOS_BREAKER
   and m.SCORE_MIN_TOTAL == 55 and m.SCORE_TRIGGER_ENABLED)
ok("V7 مزاج العملات في الحالة الدائمة", isinstance(m.STATE.get("flavor"), dict))
