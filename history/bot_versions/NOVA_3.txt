...
# ══════════════════════════════════════════════════════════════════════════════
# 1.5) طبقة الوزن V4.1 — عتبات قابلة للضبط + محاسبة أسباب الرفض
#      كل العتبات أدناه مبنية على قياس على 89,970 شمعة 5m لثلاثين زوجاً
#      (8.3 يوم) — لا على الرأي. راجع FINDINGS.md للأرقام الكاملة.
# ══════════════════════════════════════════════════════════════════════════════

# ── (1) فلتر المطاردة: أهم عقدة وحيدة في البوت كله ───────────────────────────
# ext = (سعر القرار − قمة الهيكل المكسورة) / ATR.  القياس:
#   ext ≤ -0.15 → فوز 62.0% ، R=+0.602 ، PF 2.27   (الدخول عند ارتداد تحت القمة)
#   ext >  1.0  → فوز 36.0% ، R=-0.261 ، PF 0.66   (الدخول بعد مطاردة 1×ATR فوق القمة)
# الإغلاق بـ 9.9 = بلا فلتر = سلوك V4.0 الحالي.
ENTRY_MAX_EXT_ATR = env_float("ENTRY_MAX_EXT_ATR", 0.10)

# ── (2) شرط الإزاحة الدنيا على شمعة الكسر ─────────────────────────────────────
# القياس (منفرداً): disp ≥ 1.6×ATR نقل الفوز 50.5%→54.8% و R 0.239→0.379.
MSS_MIN_DISPLACEMENT_ATR = env_float("MSS_MIN_DISPLACEMENT_ATR", 1.6)

# ── (3) شريط التطاير (حماية من أكل العمولة) ──────────────────────────────────
# عند ATR=0.30% تكلّف ذهاب-إياب 0.44R من أصل ربح 2R → لا جدوى رياضية.
ATR_PCT_MIN = env_float("ATR_PCT_MIN", 0.30)
ATR_PCT_MAX = env_float("ATR_PCT_MAX", 2.50)

# ── (4) دفتر الأوامر: من «بوابة قاتلة» إلى «حقّ نقض» فقط ─────────────────────
# اختلال أفضل مستوىين (L1) ضجيج: لا قيمة تنبؤية تقاس بصدفة ≈ 50%، لكنه كان
# البوابة التي تمصّ 60–80% من الإشارات الصالحة. الآن: يرفض فقط الشدة المعاكسة.
FLOW_VETO_IMBALANCE = env_float("FLOW_VETO_IMBALANCE", 0.40)   # رفض إن كان < 0.40
FLOW_STRONG_IMBALANCE = env_float("FLOW_STRONG_IMBALANCE", 0.55)  # للتوثيق فقط

# ── (5) صلاحية أمر الدخول (المشكلة الصامتة الأخطر) ────────────────────────────
# أمر الدخول LIMIT بـ timeInForce=GTC بلا انتهاء: يبقى على الدفتر ساعات، ثم
# يُملأ عند هبوط السعر إليه بينما ATR و EMA قد بليا. القياس على الإشارة نفسها:
#   الدخول بسعر الإغلاق   : فوز 59.6%  R=+0.948  PF 2.91
#   الدخول بـ LIMIT مُلأ لاحقاً: فوز 47.7%  R=+0.531  PF 1.83   → ΔR = −0.417
# أي أن انتظار «تحسين السعر» يختار صفقات الصفقة الأسوأ (اختيار معاكس).
ENTRY_ORDER_GRACE = env_float("ENTRY_ORDER_GRACE", 45.0)   # ثانية ثم إلغاء إن لم يُملأ
ENTRY_PRICE_OFFSET_PCT = env_float("ENTRY_PRICE_OFFSET_PCT", 0.0)  # + = تقبّل انزلاق فوق Ask
PENDING_MAX_AGE = env_float("PENDING_MAX_AGE", 20.0)   # عمر الطلقة المعلقة (ث) بدل 60
PENDING_MAX_MOVE_ATR = env_float("PENDING_MAX_MOVE_ATR", 0.6)

# ── (6) تهدئة نفس العملة ──────────────────────────────────────────────────────
# القياس: التهديد من 300ث إلى 120ث لا يغيّر الجودة إطلاقاً (PF 2.72↔2.71)
# ويضاعف العدد تقريباً. نُبقيها منخفضة لكن لا نلغيها (حماية من التكرار الآلي).

# ── (7) المخرجات: لا تُضيّق الوقف ─────────────────────────────────────────────
# تجربة تضييق الوقف من 2.0→1.25×ATR وحدها: الفوز 50.5%→39.9% و PF 1.38→1.27.
# أي أن الوقف ليس المشكلة؛ الدخول المتأخر هو المشكلة. نُبقي 2.0 ونُصحّح الهدف.
# الهدف 4.0×ATR كان معقولاً (متوسط R يرتفع مع TP من 2→4)، لذا يبقى 4.0.

# محاسب أسباب الرفض — بلا هذا لا يمكن الوزْن: أنت تعدّل في الظلام
GATE_REASONS = {"trend": 0, "mss": 0, "mss-weak": 0, "mss-stale": 0, "fvg": 0, "vol-low": 0,
                "vol-high": 0, "chase": 0, "flow-veto": 0, "spread": 0, "fvg-break": 0,
                "no-book": 0, "pending-expired": 0, "pending-moved": 0, "cooldown": 0,
                "max-pos": 0, "fired": 0, "pending": 0}
GATE_REASONS_D = {}
GATE_SINCE = time.time()


def note_gate(symbol, reason):
    """عدّاد غير متزامن رخيص: كل شمعة تُسجَّل سببها (بما فيها 'fired')."""
    GATE_REASONS[reason] = GATE_REASONS.get(reason, 0) + 1
    row = GATE_REASONS_D.setdefault(symbol, {})
    row[reason] = row.get(reason, 0) + 1


def gates_summary_text():
    """يُطبع في /status أو في سجل الإقلاع — يحوّل الوزْن من تخمين إلى قياس."""
    span = max(1.0, time.time() - GATE_SINCE) / 3600.0
    total = sum(GATE_REASONS.values())
    lines = [f"قَمع الإشارات ({span:.1f}س | {total} تقييم):"]
    for reason, cnt in sorted(GATE_REASONS.items(), key=lambda kv: -kv[1]):
        if cnt:
            lines.append(f"   {reason:16s} {cnt:7d}  {cnt / max(total, 1) * 100:5.2f}%")
    lines.append(f"   → فرص مكتملة البوابة = {GATE_REASONS.get('fired', 0) + GATE_REASONS.get('pending', 0)}"
                 f" ≈ {(GATE_REASONS.get('fired', 0) + GATE_REASONS.get('pending', 0)) / max(span, 1e-9):.1f}/ساعة")
    return "\n".join(lines)


def save_gates():
    """يُستدعى من حلقة الصيانة: يكتب القمع كل دورة ليُحلَّل لاحقاً."""
    try:
        with open(os.path.join(DATA_DIR, "gates.json"), "w") as fh:
            json.dump({"since": GATE_SINCE, "total": GATE_REASONS, "by_symbol": GATE_REASONS_D}, fh)
    except OSError:
        pass
...
# ── V4.2: حارس الحماية وسياسة الأوامر اليتيمة ────────────────────────────────
PROTECTION_GUARD = env_bool("PROTECTION_GUARD", True)          # حارس الوقف/الهدف
REARM_FALLBACK_EXIT = env_bool("REARM_FALLBACK_EXIT", False)   # True = إغلاق بدل إعادة التسليح
ORPHAN_POLICY = os.getenv("ORPHAN_POLICY", "keep").strip().lower()   # keep | cancel
...
def load_state():
    ...
    try:
        ...
        # B4: لا ننسَ المراكز بصمت — ننقل الملف التالف للاسترجاع اليدوي ونجهِر بالخطر
        try:
            backup = STATE_FILE + ".corrupt-" + str(int(time.time()))
            os.replace(STATE_FILE, backup)
            log(f"تعذر تحميل الذاكرة: {exc} — نُقلت النسخة التالفة إلى {backup}")
        except OSError:
            log(f"تعذر تحميل الذاكرة: {exc}")
        if STATE.get("positions"):
            log(f"⚠ ذاكرة جديدة بينما الحالة تحمل {len(STATE['positions'])} مركزاً — "
                f"قد تكون أوامر المنصة يتيمة؛ سيجري جردها في النبضة الأولى")
...
def on_book(symbol, data):
    ...
    try:
        ...
        if time.monotonic() > pending["until"]:
            ...
            note_gate(symbol, "pending-expired")
            return
        sig = pending["sig"]
        ask_now = float((parsed or {}).get("ask") or 0.0)
        # V4.1: منع الدخول المتأخر — كل ثانية انتظار = سعر أقدم من ATR/EMA المُقيَّمة
        ref_atr = float(sig.get("atr") or 0.0)
        ref_live = float(sig.get("live") or 0.0)
        if ask_now > 0 and ref_atr > 0 and abs(ask_now - ref_live) > PENDING_MAX_MOVE_ATR * ref_atr:
            PENDING_TRIGGERS.pop(symbol, None)
            note_gate(symbol, "pending-moved")
            return
        confirmed, ask, imbalance, _reason = book_confirms(symbol, sig["fvg_mid"])
        if confirmed:
            PENDING_TRIGGERS.pop(symbol, None)
            note_gate(symbol, "fired")
            launch_entry(sig, ask or ask_now, imbalance)
    except Exception as exc:
...
def build_signal(symbol):
    ...
    if not mss:
        note_gate(symbol, "mss")
        return None
    if int(mss.get("age", 0)) > 1:
        note_gate(symbol, "mss-stale")
        return None
    fvg = detect_fvg(h, l, atr_value)
    if not fvg:
        note_gate(symbol, "fvg")
        return None
    ...
    if ema_value is None or not (price_now > float(ema_value)):
        note_gate(symbol, "trend")
        return None
    atr_pct = (atr_value / price_now * 100.0) if price_now else 0.0
    # V4.1 بوابة التطاير: عند ATR=0.30% تكلّف العمولة 0.44R من أصل ربح 2R
    if atr_pct < ATR_PCT_MIN:
        note_gate(symbol, "vol-low")
        return None
    if atr_pct > ATR_PCT_MAX:
        note_gate(symbol, "vol-high")
        return None
    # V4.1 بوابة الإزاحة: disp ≥ 1.6×ATR رفع الفوز 50.5%→54.8% (قياس)
    if float(mss["displacement"]) < MSS_MIN_DISPLACEMENT_ATR:
        note_gate(symbol, "mss-weak")
        return None
    # V4.1 بوابة منع المطاردة (الأهم): ext = (السعر − القمة المكسورة)/ATR
    ext_atr = (price_now - float(mss["prior_high"])) / atr_value
    if ext_atr > ENTRY_MAX_EXT_ATR:
        note_gate(symbol, "chase")
        return None
    return {
        "symbol": symbol, "atr": atr_value, "atr_pct": atr_pct,
        ...
        "ext_atr": ext_atr, "created_at": time.time(),
    }


def book_confirms(symbol, fvg_mid):
    """فحص BBA اللحظي من bookTicker. V4.1: اختلال التدفق لم يعد شرطاً وجودياً.

    السبب الرقمي (قياس على 89,970 شمعة 5m لثلاثين زوجاً): اختلال أفضل مستوىين
    لا قيمة تنبؤية مقاسة له، لكنه كان البوابة التي تبتلع 60–80% من الإشارات
    الصالحة. صار يُستخدم للنقض الشديد فقط، إضافة إلى السبريد وصمود الفجوة.
    """
    book = BOOK.get(symbol)
    if not book or time.monotonic() - book["ts"] > BOOK_TTL:
        note_gate(symbol, "no-book")
    ...
    if (ask - bid) / mid * 100.0 > MAX_SPREAD_PCT:
        note_gate(symbol, "spread")
        return False, ask, imbalance, "spread"
    if ask <= fvg_mid:
        note_gate(symbol, "fvg-break")
        return False, ask, imbalance, "fvg-break"
    # V4.1: من «> MIN_BOOK_IMBALANCE دائماً» إلى حقّ نقض عند تدفق معاكس صريح
    if imbalance < FLOW_VETO_IMBALANCE:
        note_gate(symbol, "flow")
...
def on_signal_candle_closed(symbol):
    ...
    if confirmed:
        ...
        PENDING_TRIGGERS[symbol] = {"until": time.monotonic() + min(TRIGGER_WINDOW, PENDING_MAX_AGE),
                                    "sig": signal}
        note_gate(symbol, "pending")
...
def launch_entry(sig, ask, imbalance):
    ...
    if count_symbol_positions(symbol) >= MAX_POSITIONS_PER_SYMBOL:
        note_gate(symbol, "max-pos")
        return
    if time.time() - float(STATE.get("last_entry_at", {}).get(symbol, 0.0)) < SAME_SYMBOL_COOLDOWN:
        note_gate(symbol, "cooldown")
        return
    signal = dict(sig)
    # V4.1: 0.0 = عند Ask مباشرة؛ انتظار هبوط السعر اختيارٌ معاكس يكلّف 0.42R
    signal["price"] = float(ask) * (1.0 - ENTRY_PRICE_OFFSET_PCT / 100.0)
...
async def mark_cooldown(symbol):
    """B7: التبريد يُقاس بـ last_entry_at — وهو ما تقرأه بوابات الدخول فعلاً.

    في V4.1 كانت هذه الدالة تكتب STATE["cooldowns"] فقط، وهو حقل لا يقرأه
    شيء في الكود، فلم يكن لمسار DRY_RUN/التنفيذ غير المفعّل أي تبريد.
"""
    async with STATE_LOCK:
        now = time.time()
        STATE.setdefault("cooldowns", {})[symbol] = now
        STATE.setdefault("last_entry_at", {})[symbol] = now
...
async def execute_entry(signal):
    ...
    try:
        ...
        schedule_entry_expiry(entry_client_id, symbol, price)
        await save_state()
        log(f"أمر دخول LIMIT {symbol} @ {fmt_price(symbol, price)} (cid={entry_client_id}) "
            f"| ext={signal.get('ext_atr', 0):+.2f}A disp={signal.get('displacement', 0):.2f}A "
            f"flow={signal.get('imbalance', 0.5):.2f} | صلاحية {ENTRY_ORDER_GRACE:.0f}ث")
        return True
    except BinanceError as exc:
...
def schedule_entry_expiry(client_id, symbol, limit_price):
    """GTC بلا انتهاء = سمّ السكالبينج: الأمر يبقى ساعات ثم يُمْلأ عند هبوط سعره.

    يلغي أمر الدخول غير المُمْلأ بعد ENTRY_ORDER_GRACE ويمرّره لنفس مسار
    resolve_entry_order (تعبئة جزئية → مركز؛ بلا تعبئة → حذف السجل).
    """
    try:
        asyncio.get_running_loop().create_task(_entry_expiry_task(client_id, symbol))
    except RuntimeError:
        pass


async def _entry_expiry_task(client_id, symbol):
    await asyncio.sleep(max(5.0, ENTRY_ORDER_GRACE))
    record = STATE.get("pending_entries", {}).get(client_id)
    if record is None or record.get("phase") in ("done", "filled"):
        return
    order_id = record.get("order_id")
    if not order_id:
        return  # واتش دوغ الصمت (12ث) يتولّى حالة اللامعرف
    try:
        ...
        log(f"انتهاء صلاحية {symbol}: تعذر الفحص ({exc})")
        return
    if order.get("status") in ("NEW", "PARTIALLY_FILLED"):
        try:
            await REST.cancel_order(symbol, order_id)
            order = await REST.get_order(symbol, order_id)
            log(f"انتهاء صلاحية دخول {symbol}: ألغيت LIMIT لم يملأ خلال "
                f"{ENTRY_ORDER_GRACE:.0f}ث @ {fmt_price(symbol, record.get('price', 0))}")
        except BinanceError as exc:
            if exc.code != -2011:
                log(f"تعذر إنهاء صلاحية دخول {symbol}: {exc}")
            return
...
async def claim_position(pid):
    """B6/B3: مطالبة ذرية بمركز — الفائز وحده يبيع ويُحاسِب. يعيد نسخة أو None."""
    async with STATE_LOCK:
        return STATE["positions"].pop(pid, None)


async def finalize_position_event(pid, position, exit_price, exit_qty, reason, claim=False):
    """إغلاق مركز من حدث executionReport (تنفيذ OCO) — بلا أي REST.

    claim=True تخصّ المركزين المسحوبين مسبقاً بالمطيرة الذرية (PANIC/بقايا رجل).
    """
    ...
    if not claim:
        # B6: مطيرة ذرية واحدة لكل مسارات الإغلاق
        if await claim_position(pid) is None:
...
async def on_execution_report(ev):
    ...
    if meta["role"] == "entry":
        ...
        if exec_type == "TRADE" and status == "FILLED":
            ...
            # B2/B5: تعبئة جزئية بلا إتمام — الرصيد المتبقي بلا حراسة؛ ننذّر ونوسم
            await note_leg_partial(pid, position, cum_qty, last_price)
        elif status in ("CANCELED", "EXPIRED", "REJECTED") and cum_qty > 0:
            # B2: الرجل نُفِّذت جزئياً ثم انقضت → إغلاق المركز وتصفية البقايا فوراً
            avg = (cum_quote / cum_qty) if cum_qty > 0 else last_price
            await finalize_partial_leg(pid, position, avg, cum_qty, f"رجل OCO {status} بتعبئة جزئية")

async def note_leg_partial(pid, position, cum_qty, last_price):
    """B5: لا إغلاق على تعبئة جزئية — تنبيه + وسم، والحارس يتولى ما بعدها."""
    try:
        planned = D(position.get("qty", "0"))
        remaining = planned - D(cum_qty)
    except (InvalidOperation, TypeError, ValueError):
        return
    async with STATE_LOCK:
        ...
        if live is None:
            return
        live["leg_partial_qty"] = dec_str(cum_qty)
        live["leg_alert_at"] = time.time()
    log(f"تنبيه حماية {position.get('symbol')} pid={pid}: رجل OCO مُلئت جزئياً "
        f"{dec_str(cum_qty)}/{dec_str(planned)} — متبقٍ {dec_str(remaining)} بلا حراسة")


async def finalize_partial_leg(pid, position, avg_price, executed_qty, reason):
    """B2: إغلاق على تعبئة جزئية + بيع فوري للبقايا (لا يُترك المركز عارياً)."""
    planned = D(position.get("qty", "0"))
    sold = D(executed_qty)
    entry = float(position.get("entry", 0))
    qty_sold = min(float(sold), float(planned)) if planned > 0 else float(sold)
    exit_price = float(avg_price or 0)
    claimed = await claim_position(pid)          # B6: نفس المطيرة — لا سباق مع PANIC
    if claimed is None:
        return
    await finalize_position_event(pid, claimed, exit_price, qty_sold, reason, claim=True)
    remaining = (planned - sold) if planned > 0 else D("0")
    symbol = position.get("symbol", "")
    if remaining > 0:
        order = await emergency_sell(symbol, remaining, f"بقايا: {reason}")
        if order:
            qty_out, quote_out, _avg = parse_fill(order, exit_price)
            await account_salvage(symbol, float(remaining) * entry, float(quote_out or 0), qty_out,
                                  f"تصفية بقايا: {reason}")
        else:
            await add_dust(symbol, remaining, float(remaining) * entry, f"تعذر تصفية بقايا: {reason}")


async def verify_protections():
    """B1: لكل مركز — تحقق أن وقفه/هدفه قائمان على المنصة، وإلا أعد التسليح.

    السلامة: لا إعادة تسليح إلا إذا كان الرصيد الأساسي كاملاً (لم تُنفَّذ أي
    رجل). هذا يميّز «الحماية أُلغيت/ماتت» عن «رجل نُفِّذت فأُلغيت أختها»،
    فيمنع بيعاً مزدوجاً على مركز شبه مغلق.
    """
    ...
    for pid, position in list(positions.items()):
        ...
        gone = False
        try:
            olid = position.get("order_list_id")
            if olid is not None:
                await REST.order_list(order_list_id=olid)
            else:
                for order_id in [x for x in (position.get("order_ids") or []) if x is not None]:
                    await REST.get_order(symbol, order_id)
        except BinanceError as exc:
            gone = exc.code in (-2011, -2013) or exc.status in (400, 404)
        except Exception:
            gone = False                       # خطأ شبكة: ننتظر الدورة القادمة
        if not gone:
            continue
        try:
            if account is None:
                account = await REST.account()
            base = (SYMBOL_RULES.get(symbol) or {}).get("base", "")
            held = free_balance_from(account, base)
        except Exception as exc:
            log(f"حارس حماية {symbol}: تعذر فحص الرصيد ({exc})")
            continue
        planned = D(position.get("qty", "0"))
        if planned <= 0 or held < planned * Decimal("0.995"):
            log(f"حارس حماية {symbol}: وقف/هدف مفقودان والرصيد {dec_str(held)} < المركز {dec_str(planned)}"
                f" — لا إعادة تسليح (خطر بيع مزدوج)؛ يلزم تدخل يدوي pid={pid}")
            continue
        entry = D(str(position.get("entry", "0")))
        atr = float(position.get("atr_5m") or 0.0)
        if entry <= 0:
            continue
        stop_price, target_price = oco_levels(entry, atr if atr > 0 else float(entry) * 0.01)
        # إن بقي معرّف قائمة مشبوه نُلغيه أولاً: لا يجوز أن يقوم وقفان لمركز واحد.
        if position.get("order_list_id") is not None:
            try:
                await REST.cancel_oco(symbol, position["order_list_id"])
            except BinanceError:
                pass
        for old_id in [x for x in (position.get("order_ids") or []) if x is not None]:
            try:
                await REST.cancel_order(symbol, old_id)
            except BinanceError:
                pass
        if REARM_FALLBACK_EXIT:
            order = await emergency_sell(symbol, planned, "حماية مفقودة")
            if order:
                qty_out, quote_out, avg = parse_fill(order, float(entry))
                await finalize_position_event(pid, position, avg, qty_out,
                                              "إغلاق وقائي: حماية مفقودة", claim=False)
                fixed += 1
            continue
        try:
            protection = await place_initial_oco(symbol, planned, entry, stop_price, target_price)
        except BinanceError as exc:
            log(f"حارس حماية {symbol}: فشل إعادة التسليح ({exc})")
            continue
        async with STATE_LOCK:
            live = STATE["positions"].get(pid)
            if live is None:
                continue                       # حُسم المركز أثناء فحص الحماية
            live["stop"] = float(protection["stop"])
            live["target"] = float(protection["target"])
            live["order_list_id"] = protection.get("order_list_id")
            live["order_ids"] = [x.get("orderId") for x in protection.get("orders", [])
                                 if x.get("orderId") is not None]
            live["protection_type"] = protection.get("type")
            live["rearmed_at"] = time.time()
            for leg_id in live["order_ids"]:
                ORDERS[str(leg_id)] = {"role": "leg", "pid": pid, "symbol": symbol}
        await save_state()
        fixed += 1
        log(f"حارس حماية {symbol}: أُعيد تسليح OCO pid={pid} "
            f"stop={fmt_price(symbol, protection['stop'])} target={fmt_price(symbol, protection['target'])}")
    return fixed


async def scan_orphan_orders():
    """B4: جرد الأوامر التي لا تعرفها الذاكرة (بعد تلف state أو تدخل يدوي)."""
    try:
        payload = await REST.open_orders_all()
    except Exception:
        return []
    rows = payload if isinstance(payload, list) else []
    mine = set()
    for position in STATE.get("positions", {}).values():
        for leg in position.get("order_ids", []) or []:
            mine.add(str(leg))
        if position.get("order_list_id") is not None:
            mine.add(str(position["order_list_id"]))
    orphans = [r for r in rows if str(r.get("orderId")) not in mine]
    if orphans:
        log(f"أوامر يتيمة على الحساب: {len(orphans)} "
            f"({', '.join(str(o.get('symbol')) for o in orphans[:8])})")
        if ORPHAN_POLICY == "cancel":
            for row in orphans:
                try:
                    await REST.cancel_order(row.get("symbol"), row.get("orderId"))
                    log(f"أُلغي أمر يتيم {row.get('symbol')} #{row.get('orderId')}")
                except BinanceError as exc:
                    log(f"تعذر إلغاء أمر يتيم: {exc}")
    return orphans
...
async def cancel_all_orders(pop_positions=False):
    """يلغي حمايات البوت المسجلة فقط (كل مركز مستقل بمعرّفه).

    pop_positions=True يحتفظ بسلوك V4.1 (إلغاء ثم إسقاط المركز من الحالة).
    التصفية تمرّر False لأنها تطالب بالمركز بنفسها بعد اكتمال الإلغاء —
    هذا يسدّ ثقب «مركز بِيع ولم يُسجَّل» (عطّل T3 في التدقيق).
    """
    async with STATE_LOCK:
    ...
    for pid, position in positions:
        ...
        if pop_positions:
            await claim_position(pid)


async def panic_close_all():
    # B6: نُلغي الحماية أولاً (بلا إسقاط المركز). إن سبقتنا رجل OCO أثناء الإلغاء
    # يُسجَّل إغلاقها في محاسبتها، والمطيرة الذرية تحسم سباق الاستحواذ — فلا
    # بيع مزدوج ولا بيع بلا قيد.
    await cancel_all_orders(pop_positions=False)
    async with STATE_LOCK:
    ...
    for record in pending:
        ...
        try:
            ...
            pass
    for pid, position in positions:
        claimed = await claim_position(pid)
        if claimed is None:
            continue
        order = await emergency_sell(claimed.get("symbol", ""), D(claimed.get("qty", 0)), "PANIC")
        if order:
            qty, quote, avg = parse_fill(order, 0)
            await finalize_position_event(pid, claimed, avg, qty, "PANIC", claim=True)
        else:
            async with STATE_LOCK:
                STATE["positions"][pid] = claimed
            log(f"PANIC: تعذر بيع {claimed.get('symbol')} — أعيد المركز لإدارته")
...
async def manage_loop():
    ...
    while True:
        ...
        try:
            ...
            if PROTECTION_GUARD and live_execution_allowed():
                await verify_protections()
            save_gates()
        except asyncio.CancelledError:
            raise
        except Exception as exc:
...
async def run_selftest():
    ...
    print("NOVA ASYNC SCALPER V4.2 SELFTEST")
    # بوابات V4.1 مرخاة داخل الاختبار فقط: الشمعة التركيبية إزاحتها ≈ 1.0×ATR
    globals()["MSS_MIN_DISPLACEMENT_ATR"] = 0.0
    globals()["ENTRY_MAX_EXT_ATR"] = 99.0
    globals()["ATR_PCT_MIN"] = 0.0
    globals()["ATR_PCT_MAX"] = 99.0
    globals()["PENDING_MAX_MOVE_ATR"] = 99.0  # بيانات الاختبار: الدفتر لا يطابق سعر الإشارة
    results = []
    ...
    check("صيغة /status العربية الحرفية",
          lines[:12] == [
              ...
              "نسبة النجاح : 40.00%",
              "",
          ] and "BTC" not in lines[:12] and "USDT" not in lines[:12]
          and "قَمع الإشارات" in text)
...
async def amain(args):
    ...
    async with aiohttp.ClientSession(connector=connector,
        ...
        try:
            ...
            await refresh_universe()
        except (BinanceError, aiohttp.ClientError, asyncio.TimeoutError, OSError) as exc:
            raise SystemExit(f"تعذر الإقلاع على Binance: {exc}")
        for symbol in list(SYMBOLS):
            try:
                await seed_symbol(symbol)
            except Exception as exc:
