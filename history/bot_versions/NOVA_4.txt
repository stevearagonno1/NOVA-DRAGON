...
TIME_STOP_MAX_RETRIES = max(1, env_int("TIME_STOP_MAX_RETRIES", 3))
TIME_STOP_RETRY_BACKOFF = max(30.0, env_float("TIME_STOP_RETRY_BACKOFF", 300.0))

# ── V4.1: حراسة انحراف الساعة (تفادي التوقف الصامت عن التداول) ──────────────
LAG_RESYNC_THRESHOLD = max(5, env_int("LAG_RESYNC_THRESHOLD", 20))
LAG_RESYNC_COOLDOWN = max(30.0, env_float("LAG_RESYNC_COOLDOWN", 120.0))
LAG_REPORT_EVERY = max(60.0, env_float("LAG_REPORT_EVERY", 300.0))
...
LAG_REJECTS = {"count": 0, "worst": 0.0, "last_log": 0.0, "last_resync": 0.0}
ENTRY_RESERVATIONS = {}
...
def migrate_positions(raw):
    ...
    for key, position in (raw or {}).items():
        ...
        # أعلام عابرة لا يجوز أن تنجو من إعادة التشغيل (وإلا تجمّد الوقف الزمني)
        for transient_key in ("closing", "time_stop_fails", "time_stop_next_try"):
            position.pop(transient_key, None)
...
def note_lag_reject(lag_ms):
    """يسجل كل إسقاط بسبب التأخير — الوقود الذي يشغّل حارس انحراف الساعة."""
    LAG_REJECTS["count"] = int(LAG_REJECTS.get("count", 0)) + 1
    if math.isfinite(lag_ms):
        LAG_REJECTS["worst"] = max(float(LAG_REJECTS.get("worst", 0.0)), float(lag_ms))
    now = time.monotonic()
    if now - float(LAG_REJECTS.get("last_log", 0.0)) >= LAG_REPORT_EVERY:
        LAG_REJECTS["last_log"] = now
        log(f"تزامن الوقت: أُسقطت {LAG_REJECTS['count']} إشارة بتأخير "
            f"(الأسوأ {LAG_REJECTS['worst']:.0f}ms > {MAX_EVENT_LAG_MS:.0f}ms)")
...
def time_sync_ok(symbol):
    ...
    if time.monotonic() - stamp > LAG_SAMPLE_TTL:
        return False, lag
    ok = lag <= MAX_EVENT_LAG_MS
    if not ok:
        note_lag_reject(lag)
    return ok, lag


async def resync_clock_if_needed(force=False):
    """انحراف ساعة الجهاز يوقف التداول صامتاً؛ هنا نكشفه ونعيد المزامنة.

    Termux بلا NTP دائم: انزياح الساعة بضع مئات من الأجزاء يجعل كل إشارة
    تبدو متأخرة > 400ms فيتوقف البوت بلا سبب ظاهر. نعيد قياس فارق الخادم.
    """
    now = time.monotonic()
    if not force and int(LAG_REJECTS.get("count", 0)) < LAG_RESYNC_THRESHOLD:
        return False
    if now - float(LAG_REJECTS.get("last_resync", 0.0)) < LAG_RESYNC_COOLDOWN:
        return False
    LAG_REJECTS["last_resync"] = now
    previous = TIME_OFFSET_MS
    try:
        await REST.sync_time()
    except Exception as exc:
        log(f"تعذر إعادة مزامنة وقت الخادم: {exc}")
        return False
    drift = TIME_OFFSET_MS - previous
    if int(LAG_REJECTS.get("count", 0)) >= LAG_RESYNC_THRESHOLD or abs(drift) > 50:
        log(f"إعادة مزامنة الساعة: الانزياح {drift:+d}ms (الفارق الكلي {TIME_OFFSET_MS:+d}ms) "
            f"بعد {LAG_REJECTS.get('count', 0)} إسقاطاً")
    LAG_REJECTS["count"] = 0
    LAG_REJECTS["worst"] = 0.0
    return True
...
def on_kline(data):
    ...
    try:
        ...
        symbol = data.get("s")
        interval = k.get("i")
        # حصراً الفريمان المشترَك بهما — أي فريم دخيل لا يُنشئ مخزناً (منع تضخم ذاكرة)
        if interval not in (SIGNAL_INTERVAL, TREND_INTERVAL):
            return
...
def build_signal(symbol):
    ...
    if not synced:
        return None  # الإسقاط مُسجَّل داخل time_sync_ok (حارس انحراف الساعة)
        ...
        try:
            ...
            for symbol, pending in list(PENDING_TRIGGERS.items()):
                if now > pending.get("until", 0.0):
                    PENDING_TRIGGERS.pop(symbol, None)
            if int(LAG_REJECTS.get("count", 0)) >= LAG_RESYNC_THRESHOLD:
                await resync_clock_if_needed()
        except asyncio.CancelledError:
            raise
        except Exception as exc:
...
async def emergency_sell(symbol, qty, reason, shrink=True):
    """بيع سوقي فوري. shrink=False عندما تكون الكمية مخصومة الرسوم مسبقاً
    (كمية OCO) — التقليص المزدوج يترك غباراً في كل خروج زمني."""
    try:
        rule = SYMBOL_RULES[symbol]
        step = rule.get("market_step", rule["step"])
        min_qty = rule.get("market_min_qty", rule["min_qty"])
        raw = D(qty) * (Decimal("1") - D(FEE_BUFFER)) if shrink else D(qty)
        quantity = round_step(raw, step)
        if quantity < min_qty:
            return None
...
def valid_list_id(position):
    """معرّف OCO صالح فقط (Binance يعيد -1 للأوامر المفردة و0 غير وارد)."""
    raw = position.get("order_list_id")
    try:
        value = int(raw)
    except (TypeError, ValueError):
        return None
...
def current_price(symbol):
    """سعر لحظي موثوق: أفضل عرض شراء طازج، وإلا شمعة حديثة، وإلا None.

    لا يجوز اتخاذ قرار خروج بسعر قديم بعد انقطاع شبكة — إرجاع None يجعل
    الوقف الزمني يتخطى المركز بدل إغلاقه على سعر وهمي.
    """
    book = BOOK.get(symbol)
    if book and time.monotonic() - book["ts"] <= BOOK_TTL and float(book.get("bid", 0)) > 0:
        return float(book["bid"])
    buffer = CANDLES.get((symbol, SIGNAL_INTERVAL))
    if not buffer or not buffer.get("t"):
        return None
    age_ms = (time.time() * 1000 + TIME_OFFSET_MS) - float(buffer["t"][-1])
    if age_ms > INTERVAL_SECONDS[SIGNAL_INTERVAL] * 2000:
        return None  # الشموع نفسها قديمة (انقطاع) → لا سعر موثوق
    if float(buffer.get("live") or 0) > 0:
        return float(buffer["live"])
    return float(buffer["c"][-1]) if buffer.get("c") else None
...
async def cancel_position_protection(pid, position):
    """يلغي OCO الخاص بمركز قبل الإغلاق الزمني.

    يعيد (نجح, اختفت الحماية): "اختفت" تعني أن Binance لا يعرف الأمر
    (-2011/-2013) — إشارة قوية إلى أن رِجل الحماية نُفذت بالفعل، فيجب
    التحقق قبل أي بيع سوقي حتى لا نبيع مركزاً مغلقاً (بيع مزدوج).
    """
    symbol = position.get("symbol", "")
    vanished = False
    list_id = valid_list_id(position)
    if list_id is not None:
        try:
            await REST.cancel_oco(symbol, list_id)
            return True, False
        except BinanceError as exc:
            if exc.code in (-2011, -2013):
                ...
                return False, False
    ok = True
    for order_id in sorted(set(x for x in position.get("order_ids", []) if x is not None)):
        try:
...
async def resolve_position_via_rest(pid, position, reason):
    """تحقق أحادي من أرجل OCO: إن كانت إحداها منفذة نغلق المركز محاسبياً."""
    symbol = position.get("symbol", "")
    for leg_id in [x for x in position.get("order_ids", []) if x is not None]:
        try:
            order = await REST.get_order(symbol, leg_id)
        except BinanceError:
            continue
        if order.get("status") == "FILLED" and D(order.get("executedQty", "0")) > 0:
            await finalize_position_order(pid, position, order, reason)
            return True
    return False


async def restore_protection(pid, position):
    """إن أُلغيت الحماية ثم تعذّر البيع: نعيد بناء OCO فوراً — لا مركز عارٍ."""
    symbol = position.get("symbol", "")
    try:
        protection = await place_initial_oco(
            symbol, D(position.get("qty", 0)), D(str(position.get("entry", 0))),
            D(str(position.get("stop", 0))), D(str(position.get("target", 0))))
    except BinanceError as exc:
        log(f"تعذر إعادة حماية {pid}: {exc}")
        return False
    async with STATE_LOCK:
        live = STATE["positions"].get(pid)
        if live is None:
            return False
        for old_leg in live.get("order_ids", []):
            ORDERS.pop(str(old_leg), None)
        ...
        live["qty"] = dec_str(protection["qty"])
        for leg_id in live["order_ids"]:
            ORDERS[str(leg_id)] = {"role": "leg", "pid": pid, "symbol": symbol}
    await save_state()
    log(f"أُعيدت حماية {pid} بعد تعذر الإغلاق الزمني")
    return True


async def abort_time_stop(pid, reason):
    """يرفع علم الإغلاق ويؤجل المحاولة التالية (بدل عاصفة إعادة محاولة)."""
    async with STATE_LOCK:
        live = STATE["positions"].get(pid)
        if live is None:
            return 0
        fails = int(live.get("time_stop_fails", 0)) + 1
        live["time_stop_fails"] = fails
        live.pop("closing", None)
        if fails >= TIME_STOP_MAX_RETRIES:
            live["time_stop_next_try"] = time.time() + TIME_STOP_RETRY_BACKOFF
    log(f"الوقف الزمني {pid}: تعذر الإغلاق ({reason}) — محاولة {fails}/{TIME_STOP_MAX_RETRIES}")
    return fails


async def close_position_market(pid, position, reason):
    """إغلاق مركز بسعر السوق: إلغاء الحماية → تحقق من السباق → MARKET SELL."""
    symbol = position.get("symbol", "")
    cancelled, vanished = await cancel_position_protection(pid, position)
    if vanished:
        # الحماية غير موجودة على الخادم: قد تكون نُفذت — تحقق قبل أي بيع
        if await resolve_position_via_rest(pid, position, "مصالحة: OCO نُفذ قبل الوقف الزمني"):
            return True
    if not cancelled:
        await abort_time_stop(pid, "فشل إلغاء الحماية")
        return False
    # سباق: هل أغلق حدث executionReport المركزَ أثناء الإلغاء؟
    async with STATE_LOCK:
        live = STATE["positions"].get(pid)
    if live is None:
        return True  # حُسم بالفعل — لا بيع مزدوج
    price_hint = current_price(symbol) or float(position.get("entry", 0) or 0)
    if sellable_qty(symbol, D(position.get("qty", 0)), price_hint) is None:
        # كمية غير قابلة للبيع (تحت فلاتر Binance) → غبار لا حلقة لا نهائية
        async with STATE_LOCK:
            dropped = STATE["positions"].pop(pid, None)
            if dropped is not None:
                for leg_id in dropped.get("order_ids", []):
                    ORDERS.pop(str(leg_id), None)
        if dropped is not None:
            await add_dust(symbol, position.get("qty", 0), position.get("quote_qty", 0),
                           "وقف زمني: كمية دون الحد الأدنى للبيع")
        return True
    # كمية المركز أصلاً مخصومة الرسوم (كمية OCO) → لا تقليص مزدوج
    order = await emergency_sell(symbol, D(position.get("qty", 0)), reason, shrink=False)
    if not order:
        fails = await abort_time_stop(pid, "رفض البيع السوقي")
        if fails >= TIME_STOP_MAX_RETRIES:
            # ربما نُفذت الحماية فعلاً، وإلا فالمركز عارٍ → أعد بناء OCO
            if not await resolve_position_via_rest(pid, position, "مصالحة: بعد فشل الوقف الزمني"):
                await restore_protection(pid, position)
        return False
...
async def enforce_time_stops():
    ...
    async with STATE_LOCK:
        aged = [(pid, copy.deepcopy(pos)) for pid, pos in STATE.get("positions", {}).items()
                ...
                and now >= float(pos.get("time_stop_next_try", 0.0))
...
def rebuild_order_index():
    """يعيد بناء فهرس ORDERS من الذاكرة المحفوظة بعد كل إقلاع.

    بدونه لا يعرف البوت بعد إعادة التشغيل أن orderId يخص مركزاً له، فيتجاهل
    executionReport الخاص بأرجل OCO ويبقى المركز مفتوحاً في الذاكرة للأبد.
    """
    restored = 0
    for pid, position in (STATE.get("positions") or {}).items():
        symbol = position.get("symbol", "")
        for leg_id in position.get("order_ids", []) or []:
            if leg_id is None:
                continue
            ORDERS[str(leg_id)] = {"role": "leg", "pid": pid, "symbol": symbol}
            restored += 1
    for client_id, record in (STATE.get("pending_entries") or {}).items():
        order_id = record.get("order_id")
        if order_id is None:
            continue
        ORDERS[str(order_id)] = {"role": "entry", "cid": client_id,
                                 "symbol": record.get("symbol", "")}
        restored += 1
    if restored:
        log(f"أُعيد بناء فهرس الأوامر: {restored} أمراً من الذاكرة المحفوظة")
    return restored
...
async def reconcile_snapshot(reason):
    ...
    async with MANAGE_LOCK:
        ...
        rebuild_order_index()   # الأحداث بعد إعادة التشغيل تحتاج فهرساً حياً
        ...
        for pid, position in list(STATE.get("positions", {}).items()):
            try:
                ...
                if not legs and valid_list_id(position) is not None:
                    try:
                        detail = await REST.order_list(order_list_id=valid_list_id(position))
...
async def panic_close_all():
    ...
    async with STATE_LOCK:
        ...
        # حجز المراكز ذرياً: الوقف الزمني يتخطى أي مركز عليه علم closing
        for position in STATE.get("positions", {}).values():
            position["closing"] = True
        positions = [(pid, copy.deepcopy(pos))
...
async def manage_loop():
    ...
    while True:
        ...
        try:
            await resync_clock_if_needed(force=True)   # انزياح ساعة Termux
...
async def run_selftest():
    ...
    # 12د) اختبارات انحدار للثغرات المُصلَحة في مراجعة V4.1
    STATE = default_state()
    ORDERS.clear()
    STATE["positions"]["TSTUSDT#r"] = {
        "pid": "TSTUSDT#r", "symbol": "TSTUSDT", "entry": 100.0, "qty": "0.2",
        "order_list_id": 42, "order_ids": [811, 812], "opened_at": time.time()}
    STATE["pending_entries"]["cidR"] = {"symbol": "TSTUSDT", "order_id": 813,
                                        "placed_at": time.time(), "phase": "placed"}
    restored = rebuild_order_index()
    index_ok = (restored == 3 and ORDERS["811"]["pid"] == "TSTUSDT#r"
                and ORDERS["811"]["role"] == "leg" and ORDERS["813"]["role"] == "entry")
    check("انحدار: فهرس الأوامر يُعاد بناؤه بعد إعادة التشغيل", index_ok)

    transient = migrate_positions({"TSTUSDT#t": {
        "symbol": "TSTUSDT", "closing": True, "time_stop_fails": 3,
        "time_stop_next_try": time.time() + 999, "entry": 1.0}})["TSTUSDT#t"]
    check("انحدار: الأعلام العابرة لا تنجو من إعادة التشغيل",
          not any(k in transient for k in ("closing", "time_stop_fails", "time_stop_next_try")))

    check("انحدار: معرّف OCO يُقبل موجباً فقط",
          valid_list_id({"order_list_id": 900}) == 900
          and valid_list_id({"order_list_id": -1}) is None
          and valid_list_id({"order_list_id": 0}) is None
          and valid_list_id({}) is None)

    # سعر قديم لا يقود قرار خروج
    CANDLES[("TSTUSDT", SIGNAL_INTERVAL)] = {
        "t": [int(time.time() * 1000) - 50 * step_sig], "o": [100.0], "h": [101.0],
        "l": [99.0], "c": [100.0], "v": [1.0], "live": 100.0}
    BOOK.pop("TSTUSDT", None)
    stale_price = current_price("TSTUSDT")
    BOOK["TSTUSDT"] = dict(bid_heavy, ts=time.monotonic())
    fresh_price = current_price("TSTUSDT")
    check("انحدار: لا سعر (ولا وقف زمني) على بيانات قديمة",
          stale_price is None and fresh_price is not None)
    BOOK.pop("TSTUSDT", None)
    CANDLES.pop(("TSTUSDT", SIGNAL_INTERVAL), None)

    # البيع الطارئ: تقليص مزدوج للكمية أم لا
    class _SellRest:
        def __init__(self):
            self.quantities = []
        async def new_order(self, params):
            self.quantities.append(params["quantity"])
            return {"orderId": 1, "status": "FILLED", "executedQty": params["quantity"],
                    "cummulativeQuoteQty": "20"}
    sell_stub = _SellRest()
    globals()["REST"] = sell_stub
    await emergency_sell("TSTUSDT", D("0.200"), "اختبار", shrink=True)
    await emergency_sell("TSTUSDT", D("0.200"), "اختبار", shrink=False)
    globals()["REST"] = real_rest
    check("انحدار: كمية OCO تُباع كاملة (بلا تقليص مزدوج) " + str(sell_stub.quantities),
          sell_stub.quantities == ["0.199", "0.2"])

    # إلغاء حماية غير موجودة يُبلغ عن الاختفاء (تفادي البيع المزدوج)
    class _GoneRest:
        async def cancel_oco(self, symbol, order_list_id):
            raise BinanceError("Unknown order list", code=-2011, status=400)
        async def get_order(self, symbol, order_id):
            return {"status": "FILLED", "executedQty": "0.2", "cummulativeQuoteQty": "20.6"}
    globals()["REST"] = _GoneRest()
    globals()["save_state"] = _noop_save
    gone_ok, gone_vanished = await cancel_position_protection(
        "TSTUSDT#g", {"symbol": "TSTUSDT", "order_list_id": 5, "order_ids": []})
    STATE["positions"]["TSTUSDT#g"] = {"pid": "TSTUSDT#g", "symbol": "TSTUSDT",
                                       "entry": 100.0, "qty": "0.2", "order_ids": [815]}
    resolved = await resolve_position_via_rest("TSTUSDT#g", STATE["positions"]["TSTUSDT#g"],
                                               "اختبار مصالحة")
    globals()["REST"] = real_rest
    globals()["save_state"] = real_save
    check("انحدار: حماية مختفية → تحقق REST بدل بيع سوقي أعمى",
          gone_ok and gone_vanished and resolved and "TSTUSDT#g" not in STATE["positions"])

    # حارس انحراف الساعة
    LAG_REJECTS.update({"count": 0, "worst": 0.0, "last_log": time.monotonic(), "last_resync": 0.0})
    EVENT_LAG_MS["TSTUSDT"] = (MAX_EVENT_LAG_MS + 500.0, time.monotonic())
    time_sync_ok("TSTUSDT")
    counted = LAG_REJECTS["count"] == 1 and LAG_REJECTS["worst"] > MAX_EVENT_LAG_MS
    LAG_REJECTS.update({"count": 0, "worst": 0.0, "last_log": 0.0, "last_resync": 0.0})
    EVENT_LAG_MS.pop("TSTUSDT", None)
    check("انحدار: كل إسقاط تأخير يُحصى لحارس الساعة", counted)

    # فريم دخيل لا يُنشئ مخزناً
    SYMBOLS_SET.add("TSTUSDT")
    on_kline({"e": "kline", "E": int(time.time() * 1000), "s": "TSTUSDT",
              "k": {"i": "1m", "x": True, "t": 1, "o": "1", "h": "1", "l": "1",
                    "c": "1", "v": "1"}})
    check("انحدار: فريم غير مشترَك لا يلوث الذاكرة",
          ("TSTUSDT", "1m") not in CANDLES)
    STATE = default_state()
    ORDERS.clear()
