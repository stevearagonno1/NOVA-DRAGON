...
# -*- coding: utf-8 -*-
"""
NOVA V6.1 AI-HFT — Hybrid Engine: Spray + Flash-Crash Sniper + Fee-Aware Exits
==============================================================================
...
STARTUP_TEXT = "✅ NOVA V6.1 Hybrid AI-HFT Started & Connected to Binance WebSockets."
...
def handle_sniper_decision(symbol, decision):
    ...
    log(f"⚡ قنّاص الانهيار: {symbol} هبوط {decision['drop_pct']:.2f}% خلال {TICK_MEMORY_SEC:.0f}ث "
        f"+ جدار شراء {decision['wall_ratio']:.2f}x → MARKET BUY فوري")
    launch_entry(signal)
...
def handle_spray_tick(symbol):
    ...
    log(f"🚀 رشاش {symbol}: {decision['trigger']} حركة {decision['move_pct']:.2f}% "
        f"(عتبة {decision['thr']:.2f}%) تدفق {decision['flow_ratio']:.1f}x → MARKET BUY")
    launch_entry(signal)
...
def productivity_text():
    ...
    return (
        f"\n📊 إنتاجية V6.1 ({'⚠️ TESTNET' if BINANCE_ENV != 'live' else '🔴 LIVE'}):\n"
...
HELP_TEXT = (
    "🤖 <b>NOVA V6.1 Hybrid AI-HFT — رشاش + قنّاص</b>\n\n"
...
async def handle_command(text, chat):
    ...
    if command in ("/start", "/menu"):
        await send_message("🎛️ <b>NOVA V6.1 Hybrid AI-HFT</b>\nاختر عملية:", main_keyboard(), chat)
    elif command == "/help":
        await send_message(HELP_TEXT, main_keyboard(), chat)
    elif command == "/status":
        await send_message(status_text(), main_keyboard(), chat)
...
async def run_selftest():
    ...
    print("NOVA V6.1 Hybrid SELFTEST — Spray+Sniper / Fee-Aware Exits / Governor")
    results = []
...
async def amain(args):
    ...
    async with aiohttp.ClientSession(connector=connector,
        ...
        log(
            f"تشغيل NOVA V6.1 Hybrid AI-HFT [رشاش + قنّاص] | {regime_text()} | Spot فقط | "
