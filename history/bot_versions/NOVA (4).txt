...
# -*- coding: utf-8 -*-
"""
NOVA ASYNC SCALPER V5.2 ELITE — Human-Like Compensatory Scoring (Risk-Parity)
=================================================================================
...
3) EMA_TREND_PERIOD = 100 | عتبات التدفق 1.6 / 1.3 / 1.15
   | ATR_MULT_NORMAL = 2.5 | RR_RATIO = 1.4 (هدف 3.5×ATR) | وقف زمني 900ث.
...
STARTUP_TEXT = "✅ NOVA Async V5.1 ELITE Started & Connected to Binance WebSockets."
...
ATR_MULT_NORMAL = env_float("ATR_MULT_NORMAL", 2.5)   # V5.2: مساحة تنفّس لذيول 5m
...
RR_RATIO = env_float("RR_RATIO", 1.4)                          # V5.2: TP = SL × 1.4 (أرباح أسرع)
...
TIME_STOP_MIN_PROFIT_PCT = env_float("TIME_STOP_MIN_PROFIT_PCT", 0.15)  # ربح ≥0.15% يُترك لـ OCO
...
def compute_score(has_mss, has_fvg, trend_ok, imb, trend_slope_up=False,
    ...
    (imb > MOMENTUM_FLOW_EXTREME أو vol_ratio > MOMENTUM_VOL_EXTREME) تُضاف
    ...
    momentum = bool(imb > MOMENTUM_FLOW_EXTREME or vol_ratio > MOMENTUM_VOL_EXTREME)
...
def _evaluate_frame(symbol):
    ...
    score, components, triggered, parts = compute_score(
        mss is not None, fvg is not None, trend_ok, imb, slope_up, vol_ratio)
...
def calculate_dynamic_risk(entry, atr_value, vr):
    ...
    mult, regime = atr_multiplier(max(0.0, vr))
    sl_distance = max(atr_value, 0.0) * mult
    cap = entry * SL_MAX_PCT / 100.0
    # أرضية صلبة: وقف لا يقل عن SL_MIN_PCT% من السعر (يمنع OCO منحلاً عند سعر
    # الدخول حين ATR صغير جداً)، وسقف صلب SL_MAX_PCT% ضد الانهيارات المفاجئة.
...
async def complete_entry(client_id, record, executed_qty, avg_price, quote_filled):
    ...
    try:
        ...
        await save_state()
        log(f"دخول V5 {symbol} pid={pid} entry={fmt_price(symbol, avg_price)} qty={position['qty']} "
...
async def force_exit_position(pid, position, reason):
    """خروج استثنائي موحّد (تصفية/وقف زمني): مطالبة → إلغاء OCO → بيع MARKET.
    ...
    بعد إلغاء OCO (ربما نُفذ رجل الوقف/الهدف أثناء الإلغاء).
    """
    ...
    try:
        ...
        try:
            ...
            log(f"تعذر إلغاء حماية {pid} قبل الخروج: {exc}")
        # تحقق أخير بعد الإلغاء: إن نُفذ OCO بالفعل أُغلق المركز فلا بيع
        async with STATE_LOCK:
            if STATE.get("positions", {}).get(pid) is None:
                log(f"خروج {pid}: أُغلق عبر OCO أثناء الإلغاء؛ تجاوز البيع")
                return False
        ...
        if order:
            ...
            if qty and qty > 0:
                await finalize_position_event(pid, position, avg, qty, reason)
                return True
        # فشل البيع (رصيد/شبكة): أفرج المطالبة ليتسنى للمصالحة/المحاولة التالية
        async with STATE_LOCK:
            EXIT_CLAIMED.discard(pid)
        log(f"خروج {pid}: تعذر البيع ({reason})؛ يُترك للمصالحة")
        return False
    except Exception as exc:
        async with STATE_LOCK:
            EXIT_CLAIMED.discard(pid)
...
def context_text():
    ...
    return (
        ...
        f"الوقف: ATR × {ATR_MULT_SLOW}/{ATR_MULT_NORMAL}/{ATR_MULT_FAST} بحسب VR، بسقف {SL_MAX_PCT}% | TP = {RR_RATIO:g}×SL\n"
        f"الحجم: مخاطرة {TARGET_RISK_USD}$ لكل صفقة (سقف مركز {MAX_POSITION_NOTIONAL_USD}$)\n"
...
HELP_TEXT = (
    "🤖 <b>NOVA ASYNC SCALPER V5.1 ELITE — Graded Scoring + Risk Parity</b>\n\n"
    ...
    "محرك تنقيط متدرّج بدل AND: MSS 35 + FVG 25 + EMA50 20/10 + Flow 8/14/20 + حجم ≤5 — تنفيذ ≥ 72.\n"
    "قاطع سياق كلي: BTCUSDT@kline_1m يمنع الشراء إذا هوى BTC ≥ 0.5% خلال 3 دقائق.\n"
    "فلاتر جودة: شمعة ذيلية (ذيل > 60%)، سوق ميت (ATR% < 0.10%)، تقلب جنوني (VR > 3)،\n"
    "ومطاردة قمة (امتداد > 6×ATR عن EMA50) — كلها تمنع الدخول الرديء.\n"
    "وقف ديناميكي: ATR × 1.2/1.8/2.5 بحسب VR (سوق سريع يوسّع، مستقر يضيّق)، بسقف 3% — الهدف 2× الوقف صارم.\n"
    "حجم مبني على المخاطرة: 2$ لكل صفقة (سقف مركز 250$) — المخاطرة ثابتة مهما اتسع الوقف.\n"
    "وقف زمني: مركز بلا زخم 15 دقيقة يُصفى بسعر السوق (جوهر السكالبينج).\n"
    ...
    "OCO ثابت Set & Forget. Dust→BNB كل 24 ساعة. /context يعرض قمع أسباب المنع.\n"
...
async def handle_command(text, chat):
    ...
    if command in ("/start", "/menu"):
        await send_message("🎛️ <b>NOVA ASYNC SCALPER V5.1 ELITE</b>\nاختر عملية:", main_keyboard(), chat)
    elif command == "/help":
        await send_message(HELP_TEXT, main_keyboard(), chat)
    elif command == "/status":
        await send_message(status_text(), main_keyboard(), chat)
...
async def time_stop_loop():
    ...
    15 دقيقة غالباً انتهت صلاحية فكرته. نلغي OCO ونبيع MARKET (Set & Forget
    لا يُمسّ في المسار العادي؛ هذا تدخل استثنائي بفاصل زمني واسع).
    """
    while True:
        ...
        try:
            ...
            for pid, position in open_positions:
                ...
                # نُصفّي فقط إذا لم يكن الربح مقبولاً (الرابح يُترك لـ OCO يجني هدفه)
...
async def run_selftest():
    ...
    check(f"مصفوفة المخاطرة: ATR×{ATR_MULT_NORMAL:g} و RR {RR_RATIO:g} ووقف زمني {TIME_STOP_SEC}ث",
          ATR_MULT_NORMAL == 2.5 and RR_RATIO == 1.4 and TIME_STOP_SEC == 900
          and EMA_TREND_PERIOD == 100)
    ...
    check(f"معاملات ATR: {ATR_MULT_FAST:g} سريع / {ATR_MULT_SLOW:g} مستقر / {ATR_MULT_NORMAL:g} طبيعي",
          abs(r_fast["sl_distance"] - ATR_MULT_FAST) < 1e-9 and r_fast["regime"] == "fast"
          and abs(r_slow["sl_distance"] - ATR_MULT_SLOW) < 1e-9 and r_slow["regime"] == "slow"
          and abs(r_norm["sl_distance"] - ATR_MULT_NORMAL) < 1e-9 and r_norm["regime"] == "normal")
    ...
    check(f"TP = Entry + {RR_RATIO:g}×SL صارم (1:{RR_RATIO:g} دائماً)",
          abs(r_fast["tp"] - (100.0 + RR_RATIO * ATR_MULT_FAST)) < 1e-9
          and abs(r_cap["tp"] - (100.0 + RR_RATIO * 3.0)) < 1e-9
          and abs((r_fast["tp"] - 100.0) / (100.0 - r_fast["sl"]) - RR_RATIO) < 1e-9)
    stop, target = oco_levels(Decimal("100"), 1.0, 2.0)
    check("oco_levels(مع VR) تتوافق مع المصفوفة",
          stop == Decimal("97.5") and target == Decimal("103.5"))
    ...
    market_ok = (entry_ok and len(stub.order_calls) == 1
                 ...
                 and abs(float(position.get("target", 0)) - 103.5) < 1e-9
...
async def amain(args):
    ...
    async with aiohttp.ClientSession(connector=connector,
        ...
        log(
            f"تشغيل NOVA ASYNC SCALPER V5.0 [تنقيط موزون] | {regime_text()} | Spot فقط | "
