...
=====================================================================
يُنسّق كل المهام (WebSockets, KAMA, Naive-Bayes, تنفيذ, تليجرام) بحلقة asyncio واحدة.

تدفق لكل عملة (على إغلاق شمعة 1m):
  DATA → KAMA (اتجاه + قوة) → Naive-Bayes (Bull ≥ 0.55, Diverged = فيتو)
       → تنفيذ Testnet + تقرير تليجرام

(مبسّط: أُزيلت فلاتر Epsilon / Premium-Discount / Swing-Death / Flow،
 ومسار Sweep→MSS→FVG، و Six-Factor Score + الحجم المتدرّج.)
...
from nb import NaiveBayes, NB_MIN_ENTRY_PROB
import risk as RISK
...
class NovaEngine:
    ...
    async def _run_pipeline(self, st: SymbolState):
        ...
        # ── 1) KAMA — فلتر الاتجاه الوحيد (اتجاه + قوة) ──────
        ...
        # ── 2) Naive-Bayes — المُصنّف الوحيد للدخول ──────────
        # (التدريب تم في _process_kline؛ هنا نقرأ الاحتمال المحسوب فقط)
        if not self._entry_gate(st):
            return

        # ── 3) تنفيذ Testnet ─────────────────────────────────
        await self._open_trade(st, st.nb_class, st.nb_posts.get("Bull", 0.0))

    def _entry_gate(self, st: SymbolState) -> bool:
        """KAMA + Naive-Bayes — فلترا الدخول الوحيدان."""
        ...
        if nb_class == "Diverged":
            ...
            return False
        if not st.nb.ready or nb_bull < NB_MIN_ENTRY_PROB:
            self.filters_hits["nb"] += 1
            return False
        return True
    ...
    async def _open_trade(self, st, nb_class, nb_bull):
        cfg = self.cfg
        ...
        if self._has_position(st):
            return  # مركز مفتوح لنفس العملة — لا دخول جديد
        if not cfg.is_testnet:
            ...
            self.trades_book.append({"t": time.time(), "symbol": st.symbol, "entry": 0.0,
                                     ...
                                     "net": 0.0, "reason": "live disabled", "score": nb_bull,
                                     "nb": nb_class})
            return
        ...
        self.positions[st.symbol] = {
            ...
            "open_time": time.time(), "score": nb_bull, "nb": nb_class,
            "confidence": nb_bull,           # ثقة NB (0–1) — لم تُستخدم بعد في الإدارة
            "be_armed": False, "trailing": False,
        }
        sig = {"symbol": st.symbol, "price": entry, "qty": qty,
               "score": nb_bull * 100.0,            # ثقة NB على مقياس 0–100
               "tier": f"NB {nb_bull * 100.0:.0f}%",
               "nb": nb_class,
