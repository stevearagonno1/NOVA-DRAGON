# -*- coding: utf-8 -*-
"""L0017 — دفعة الجرد الأولى: نخبة الدخول الخمس على 5 سنوات (بروتوكول خط الإنتاج)

الأمر: القرار D-0046 (إذن القائد 2026-09-20) + عقد التأسيس
       docs/lanes/L0017-inventory-batch1-elite-entries.md

البروتوكول (على حرفية run_breakers.py / L0009 — لا انحراف في الحكم):
  ...
  - بوابة خط الإنتاج على الاختبار: net>0 AND PF≥1.3 AND trades≥30
    (بوابة القسم ١٦ الكاملة ≥100 صفقة تبقى للاعتماد النهائي لا لطابور التركيبات — نص العقد)
  - التكاليف 0.13% لكل طرف · 20$ للصفقة (الدستور §27 / D-0044) · شراء فقط 🔒
  - الخروج: المطاردة ثنائية السرعة بالتركيبة الذهبية trig=0.0025 lock=0.0045 wide=0.0020 tight=0.0008
    — ثابت المختبر لهذه الدفعة، لا يُسوَّى (نص العقد)
  - الكاناري بوابة سابقة إلزامية · حتمية: بلا عشوائية · الجداول بعلامة utf-8-sig (D-0011)

انحراف موثق واحد (منسوخ نصاً من L0009، لا يغيّر الحكم):
  مصفوفة المؤشرات تُحسب على الإطار المدفأ كاملاً ثم تُقص الإشارة للنافذة — مبرره أن
  مؤشرات 200 شمعة (sma200/بوابة الساعة) تخسر رأس التدريب لو حُسبت على المقطع.

المخرجات: history/research/hyp_lab_out/L0017{,-tf1h,-tf4h}/<EXP>/<sym>/{sweep_train,test_selected}.csv
...
import L0017_F_192_ext_entry as E192      # noqa: E402
import L0017_F_001_rsi2_snap as E001      # noqa: E402
import L0017_F_008_turtle_soup as E008    # noqa: E402
import L0017_F_015_zscore_regime as E015  # noqa: E402
import L0017_F_165_score_entry as E165    # noqa: E402

EXPS = {
    "F_192_ext_entry": E192,
    "F_001_rsi2_snap": E001,
    "F_008_turtle_soup": E008,
    "F_015_zscore_regime": E015,
    "F_165_score_entry": E165,
}
...
GATE_PF, GATE_TRADES = 1.3, 30          # بوابة خط الإنتاج (نص العقد)
MIN_TRAIN_TRADES = 30                   # حد عدم التحلل على التدريب
TRADE_COLS = ["symbol", "exp", "entry_time", "exit_time", "entry", "exit",
              "notional", "pnl", "r_mult", "exit_side", "bars_held"]
TF_OUT = {"5m": "L0017", "1h": "L0017-tf1h", "4h": "L0017-tf4h"}

# الخروج الذهبي المجمَّد لهذه الدفعة (نص العقد)
...
def signals_for(exp: str, df_full: pd.DataFrame, window: pd.DataFrame, combo: dict) -> pd.Series:
    """الإشارة على الإطار المدفأ كاملاً ثم تُقص للنافذة (الانحراف الموثق)."""
...
def parse_args(argv=None):
    ap = argparse.ArgumentParser(description="L0017 — دفعة الجرد الأولى: نخبة الدخول الخمس")
    ap.add_argument("--tf", choices=["5m", "1h", "4h", "all"], default="5m")
...
def main() -> int:
    ...
    # 0) 🔒 الكاناري بوابة إلزامية سابقة
    ...
    if not args.skip_canary:
        ...
        if canary.get("canary") != "ok":
            print("الكاناري فشل — إيقاف L0017 (بوابة سلامة).", file=sys.stderr)
    ...
    # 1) تحميل مرة واحدة لكل رمز ثم تعبئة كل الفريمات
    ...
    for tf in tfs:
        ...
        lines = [f"# L0017 — دفعة الجرد الأولى — أفضل (اختبار) لكل رمز — شبكة {tf}",
