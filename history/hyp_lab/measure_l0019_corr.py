# -*- coding: utf-8 -*-
"""L0019 — قياس الترابط اليومي لعابري الدفعة الثانية مع سلسلة محفظة الـ13 المودعة.
...
الطريقة (نفس daily_corr في build_portfolio_13.py وmeasure_l0017_corr.py حرفاً):
  عوائد يومية = مجموع pnl الصفقات المغلقة في اليوم · الترابط = بيرسون على الأيام المشتركة.
  سلسلة المحفظة المرجعية = اتحاد صفقات L0010 (الستة) و L0013 (السبعة) على 4h المودعة.
...
def load_portfolio_13() -> tuple[pd.Series, int, int]:
    frames = []
...
def load_l0019_passers() -> dict[str, pd.DataFrame]:
    out = {}
    for tf, lane in (("1h", "L0019-tf1h"), ("4h", "L0019-tf4h")):
        base = OUTDIR / lane
        if not base.exists():
            continue
...
def main() -> int:
    ...
    passers = load_l0019_passers()
    print(f"عابرو L0019: {len(passers)} خلية (فرضية×رمز×فريم)\n", flush=True)

    rows = []
    for key, d in sorted(passers.items()):
        exp, sym, tf = key.split("|")
        s = daily(d)
        j = pd.concat([s.rename("l0019"), pf_daily.rename("pf13")], axis=1).dropna()
        c = float(j["l0019"].corr(j["pf13"])) if len(j) >= 20 else float("nan")
